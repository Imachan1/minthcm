...
if ( isset($_REQUEST['return_type']) && $_REQUEST['return_type'] == 'report' ) {
   ...
   if ( !empty($_REQUEST['prospect_list_id']) and ! empty($_REQUEST['prospect_ids']) ) {
      add_prospects_to_prospect_list(
              $_REQUEST['prospect_list_id'], $_REQUEST['prospect_ids']
      );
   } else {
      $parent = BeanFactory::getBean($_REQUEST['module'], $_REQUEST['record']);
      add_to_prospect_list(
              urldecode($_REQUEST['subpanel_module_name']), $_REQUEST['parent_module'], $_REQUEST['parent_type'], $_REQUEST['subpanel_id'], $_REQUEST['child_id'], $_REQUEST['link_attribute'], $_REQUEST['link_type'], $parent
      );
   }
   ...
   if ( !empty($_REQUEST['select_entire_list']) && $_REQUEST['select_entire_list'] != 'undefined' && isset($_REQUEST['current_query_by_page']) ) {
      ...
      //viewTools start #46476 - Błąd podczas powiązywania większej liczby kontrahentów z kampanią sprzedażową
      if ( $current_query_by_page_array === null ) {
         $current_query_by_page_array = sugar_unserialize(base64_decode($current_query_by_page));
      }
      //viewTools end #46476 - Błąd podczas powiązywania większej liczby kontrahentów z kampanią sprzedażową
