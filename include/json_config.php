...
class json_config {
   ...
   public function get_static_json_server(
      $configOnly = true,
      $getStrings = false,
      $module = null,
      $record = null,
   ...
   public function getFocusData($module, $record) {
      ...
      if ( empty($module) ) {
         ...
         return "\n" . $this->global_registry_var_name .
            '["focus"] = {"module":"' . $module . '",users_arr:[],fields:{"id":"-1"}}' . "\n";
      }
   ...
   public function meeting_retrieve($module, $record) {
      ...
      if ( $module != 'Project' && $module != 'AM_ProjectTemplates' ) {
         ...
         if ( $module == 'Meetings' || $module == "Calls" ) {
            ...
            $resources_relationship_name = 'resources';
            $focus->load_relationships($candidates_relationship_name);
            $focus->load_relationships($resources_relationship_name);
            $additional_beans = array_merge(
               $focus->get_linked_beans($candidates_relationship_name, 'Candidates'),
               $focus->get_linked_beans($resources_relationship_name, 'Resources')
            );
         }
   ...
   public function getStringsJSON($module) {
      ...
      global $json;
      $str = "\n" .
         $this->global_registry_var_name . "['calendar_strings'] =  {\"dom_cal_month_long\":" .
         $json->encode($mod_list_strings['dom_cal_month_long']) . ",\"dom_cal_weekdays_long\":" .
         $json->encode($mod_list_strings['dom_cal_weekdays_long']) . "}\n";
      if ( empty($module) ) {
         $module = 'Home';
      }
      $currentModule = $module;
      $mod_strings = return_module_language($current_language, $currentModule);
      return $str . "\n" . $this->global_registry_var_name . "['meeting_strings'] =  " .
         $json->encode($mod_strings) . "\n";
   }
