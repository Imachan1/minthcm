   ...
   public function display($showContainer = true, $forceTabless = false) {
      ...
      foreach ( $tabs as $t => $tab ) {
         ...
         if ( $tabs_properties[$t]['expanded_subpanels'] ) {
            ...
            //$subPanelDef = $this->subpanel_definitions->layout_defs['subpanel_setup'][$tab]; //viewTools #60768
         }

// View Tools #56822 #60768 START
         global $sugar_config;
         if ( in_array($sugar_config["subpanel_count_method"], [ "count", "limited_count" ]) ) {
            $subPanelDef = $this->subpanel_definitions->layout_defs['subpanel_setup'][$tab];
            if ( $sugar_config["subpanel_count_method"] == "limited_count" ) {
               $subPanelDef["count_limit"] = 11;
            }
            ...
            if ( $count === 0 ) {
               ...
               if ( $sugar_config["subpanel_count_method"] == "limited_count" && $count > 10 ) {
                  $countStr = '10+';
               } else {
                  $countStr = $count . '';
               }
            ...
            if ( $count > 0 ) {
               $tabs_properties[$t]['title'] .= ' <span class="counter">('.$countStr.')</span>';
            }
         }
