...
*}
<select name='parent_type' tabindex="{{$tabindex}}" id='parent_type' title='{{$vardef.help}}' class="vt_formulaSelector" {if $locked_field == "parent_type"}disabled{/if} {{if !empty($displayParams.accesskey)}} accesskey='{{$displayParams.accesskey}}' {{/if}}
...
{/if}
<input type="text" name="{{sugarvar key='name'}}" id="{{sugarvar key='name'}}" class="vt_formulaSelector{{if $vardef.vt_enforced!='false' and $vardef.vt_calculated!=''}} vt_enforced{{/if}} sqsEnabled" tabindex="{{$tabindex}}"
       size="{{$displayParams.size}}" {if $keepParent}value="{{sugarvar key='value'}}"{/if} autocomplete="off" {if $locked_field == "parent_type"}readonly{/if}
...
<input type="hidden" class="vt_formulaSelector" name="{{sugarvar memberName='vardef.id_name' key='name'}}" id="{{sugarvar memberName='vardef.id_name' key='name'}}"  
        ...
        <script type="text/javascript">
            if ( typeof (changeParentQS) == 'undefined' ) {
               function changeParentQS( field ) {
                  ...
                  if ( (typeof (disabledModules) != 'undefined' && typeof (disabledModules[new_module]) != 'undefined') || {/literal}"{{$displayParams.readOnly}}" == "readonly" || "{$locked_field}" == "parent_type" ) {* viewTools *}       {literal} {
