 ...
 */
// MintHCM start #50211 added minutesStep
function Datetimecombo( datetime, field, timeformat, tabindex, showCheckbox, checked, allowEmptyHM, minutesStep ) {
...
// MintHCM end #50211
   ...
   if ( datetime != '' )
   {
      ...
      // MintHCM round to 5 min
      if ( this.mins % 5 != 0 ) {
         this.mins = Math.ceil( this.mins / 5 ) * 5;
      }
      // MintHCM End 
   }
...
Datetimecombo.prototype.html = function ( callback ) {
   var text = '<span><select class="datetimecombo_time" size="1" id="' + this.fieldname + '_hours" tabindex="' + this.tabindex + '" onchange="combo_' + this.fieldname + '.update(); viewTools.form.blur($(\'#' + this.fieldname + '\')); ' + callback + '">';
   ...
   text += '\n&nbsp;<select class="datetimecombo_time" size="1" id="' + this.fieldname + '_minutes" tabindex="' + this.tabindex + '" onchange="combo_' + this.fieldname + '.update(); viewTools.form.blur($(\'#' + this.fieldname + '\')); ' + callback + '">';
   if ( this.allowEmptyHM ) {
      text += '\n<option></option>';
   }
   // MintHCM start #50211
   var minutes_step = parseInt( this.minutesStep );
   var step = !isNaN( minutes_step ) ? minutes_step : 15;
   for ( var minute = 0; minute <= 59; minute += step ) {
      var parsed_number = (minute).toLocaleString( undefined, {minimumIntegerDigits: 2, useGrouping: false} );
      text += '\n<option value="' + parsed_number + '" ' + (this.mins == minute ? "SELECTED" : "") + '>' + parsed_number + '</option>';
   }
   // MintHCM end #50211
   text += '\n</select>';
   if ( this.hasMeridiem ) {
