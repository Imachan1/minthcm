...
<input type="hidden" class="vt_formulaSelector{{if $vardef.vt_enforced!='false' and $vardef.vt_calculated!=''}} vt_enforced{{/if}} DateTimeCombo" id="{{$idname}}" name="{{$displayParams.originalFieldName}}" value="{$fields[{{sugarvar key='name' stringFormat=true}}].value}" 
       {{if $vardef.vt_dependency!=''}} data-dependency="{{$vardef.vt_dependency}}" {{/if}}
       {{if $vardef.vt_calculated!=''}} data-calculated="{{$vardef.vt_calculated}}" {{/if}}
       {{if $vardef.vt_required!=''}} data-required="{{$vardef.vt_required}}" {{/if}}
       {{if $vardef.vt_readonly!=''}} data-readonly="{{$vardef.vt_readonly}}" {{/if}}
       {{if $vardef.vt_validation!=''}} data-validation="{{$vardef.vt_validation}}" {{/if}}>
{{else}}
<input type="hidden" class="vt_formulaSelector{{if $vardef.vt_enforced!='false' and $vardef.vt_calculated!=''}} vt_enforced{{/if}} DateTimeCombo" id="{{$idname}}" name="{{$idname}}" value="{$fields[{{sugarvar key='name' stringFormat=true}}].value}" 
       {{if $vardef.vt_dependency!=''}} data-dependency="{{$vardef.vt_dependency}}" {{/if}}
       {{if $vardef.vt_calculated!=''}} data-calculated="{{$vardef.vt_calculated}}" {{/if}}
       {{if $vardef.vt_required!=''}} data-required="{{$vardef.vt_required}}" {{/if}}
       {{if $vardef.vt_readonly!=''}} data-readonly="{{$vardef.vt_readonly}}" {{/if}}
...
<script type="text/javascript">
    ...
    var combo_{{$idname}} = new Datetimecombo( "{$fields[{{sugarvar key='name' stringFormat=true}}].value}", "{{$idname}}", "{$TIME_FORMAT}", "{{$tabindex}}", '{{$displayParams.showNoneCheckbox}}', false, true, '{{$displayParams.minutesStep}}' );
    {* MintHCM end #50211 *}
