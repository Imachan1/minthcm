...
<span class="dateTime">
    ...
    <input class="date_input vt_formulaSelector{{if $vardef.vt_enforced!='false' and $vardef.vt_calculated!=''}} vt_enforced{{/if}}" autocomplete="off" type="text" name="{{$idname}}" id="{{$idname}}" value="{$date_value}" title='{{$vardef.help}}' {{$displayParams.field}} tabindex='{{$tabindex}}' {{if !empty($displayParams.accesskey)}} accesskey='{{$displayParams.accesskey}}' {{/if}} 
           size="11" maxlength="10" onchange="viewTools.form.blur( $( '#{{$idname}}' ) );"
