...
*}
<input type="text" name="{{sugarvar key='name'}}"  class="vt_search_relate_input{{if empty($displayParams.class) }} sqsEnabled {{else}} {{$displayParams.class}} {{/if}}" {{if !empty($tabindex)}} tabindex="{{$tabindex}}" {{/if}}  id="{{sugarvar key='name'}}" size="{{$displayParams.size}}" value="{{sugarvar key='value'}}" title='{{$vardef.help}}' autocomplete="off" {{$displayParams.readOnly}} {{$displayParams.field}}>
