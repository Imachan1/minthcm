        ...
        {{foreach name=rowIteration from=$panel key=row item=rowData}}
            ...
            {{foreach name=colIteration from=$rowData key=col item=colData}}
                ...
                {{if $smarty.foreach.colIteration.total > 1 && $colData.colspan != 3}}
                    <div class="col-xs-12 col-sm-6 edit-view-row-item" data-field="{{$colData.field.name}}" {if $fields.$field_name.acl == 0}style="display: none;"{/if}>
                {{else}}
                    <div class="col-xs-12 col-sm-12 edit-view-row-item" data-field="{{$colData.field.name}}" {if $fields.$field_name.acl == 0}style="display: none;"{/if}>
                {{/if}}
