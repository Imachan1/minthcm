...
// mint - custom functions:
CAL.getItemStatusValue = function (module, status) {
   module = (module || '').toLowerCase();
   var appListStrings = CAL.getAppListStrings(module + '_status_dom');
   return appListStrings && appListStrings[status] || '';
};
CAL.getAppListStrings = function (list) {
   return SUGAR.language.languages.app_list_strings
      && SUGAR.language.languages.app_list_strings[list]
      || void 0;
};
CAL.getModuleLabel = function (module, singular) {
   var list = CAL.getAppListStrings('moduleList' + (singular ? 'Singular' : ''));
   return list && list[module] || void 0;
};
// mint end - custom functions;
...
CAL.reset_edit_dialog = function () {
   var e;
   //Mint start
   document.forms["CalendarEditView"].elements["current_module"].value = "WorkSchedules";
   CAL.get("radio_workschedule").removeAttribute("disabled");
   //Mint end
   ...
   //Mint start
   CAL.get("radio_meeting").checked = false;
   CAL.get("radio_workschedule").checked = true;
   //Mint end
   ...
   //Mint start
   CAL.GR_update_focus("WorkSchedules", "");
   //Mint end
...
CAL.fill_repeat_data = function () {
   // mint - 2lines below has been modiefied:
   var cbr = ~['Meetings', 'Calls', 'WorkSchedules'].indexOf(CAL.get("current_module").value);
   if (CAL.enable_repeat && cbr) {
...
CAL.load_form = function (module_name, record, edit_all_recurrences, cal_event) {
   ...
   //   var e;
   //   var to_open = true;
   //   if ( module_name != "Meetings" && module_name != "Calls" && module_name != "WorkSchedules" ) {
   //      to_open = false;
   //   }
   ...
   //               // mint:
   //               if ( mod_name == "WorkSchedules" ) {
   //                  CAL.get( "radio_workschedule" ).checked = true;
   //                  $( "#scheduler" ).css( "display", "none" );
   //                  CAL.get( "edit_all_recurrences_block" ).style.display = "none";
   //               } else {
   //                  $( "#scheduler" ).css( "display", "" );
   //               }
   //               if ( res.edit == 1 ) {
   //                  CAL.record_editable = true;
   //               } else {
   ...
   //                  // mint:
   //                  (window.onCalendarFormLoad || new Function)();
   //               }, 500 );
   //            } else
   //               alert( CAL.lbl_error_loading );
   //         },
...
CAL.set_btn_invites = function (mod_name) {
   //Mint start
   if (mod_name == 'WorkSchedules') {
      CAL.get("btn-send-invites").disabled = true;
      CAL.get("btn-send-invites").hidden = true;
      $("#scheduler").css("display", "none");
   } else {
      CAL.get("btn-send-invites").disabled = false;
      CAL.get("btn-send-invites").hidden = false;
      $("#scheduler").css("display", "");
...
CAL.dialog_create = function (date, end_date, user_id) {
   ...
   var url = "index.php?module=" + module_name + "&action=EditView&return_action=index&redirected_from_calendar=1&return_module=" + return_module + "&date_start=" + date + "&date_end=" + end_date + "&assigned_user_id=" + user_id;
   window.parent.postMessage(new URL(url, document.location).href); // Mint-Vue
   // window.location.assign(url);
...
CAL.show_additional_details = function (id) {
   ...
   // mint - hide edit button if work schedule is closed:
   if (module_name == "WorkSchedules" && $('#' + id).attr('status') == "closed") {
      (function () {
         var sel = '.ui-dialog-title a[href*="index.php?action=EditView&'
            + 'module=WorkSchedules&record=' + id + '"]',
            el = [].pop.call(document.querySelectorAll(sel));
         if (el)
...
$($.fullCalendar).ready(function () {
   ...
   $.each(calendar_items, function (user_id, user_calendar_activities) {
      ...
      $.each(user_calendar_activities, function (index, element) {
         ...
         if (element.module_name != "Meetings" && element.module_name != "Calls"/* Mint start */ && element.module_name != "WorkSchedules" /* Mint end */) {
            valueToPush['editable'] = false;
            //Mint start #60436
         } else if (element.module_name === "WorkSchedules" && element['status'] == 'closed') {
            valueToPush['editable'] = false;
            //Mint end 60436
         }
         // Mint start
         if (element.module_name == 'WorkSchedules') {
            // #42688 start
            ...
            if (element.module_name == 'Meetings') {
               var meeting_status_dom = SUGAR.language.get('app_list_strings', 'meeting_status_dom');
               valueToPush["title"] = element['name'] + ' (' + meeting_status_dom[element['status']] + ')';
            } else if (element.module_name == 'Calls') {
               var call_status_dom = SUGAR.language.get('app_list_strings', 'call_status_dom');
               valueToPush["title"] = element['name'] + ' (' + call_status_dom[element['status']] + ')';
            }
            // Mint #43244 END
   ...
   function constructCalendar(user_id, all_events) {
      ...
      $('#calendar' + user_id).fullCalendar({
         ...
         eventResize: function (event, delta, revertFunc) {
            ...
            // mint start
            if (event.start.get('year') != event.end.get('year') || event.start.get('month') != event.end.get('month') || event.start.get('day') != event.end.get('day')) {
               alert(SUGAR.language.translate('app_strings', 'LBL_PLAN_ONE_DAY'));
         ...
         eventRender: function (event, element) {
            var url = 'index.php?to_pdf=1&module=Home&action=AdditionalDetailsRetrieve&bean=' + event.module + '&id=' + event.id + '&show_buttons=1';
            ...
            if ($('#cal_module').val() != "Home" && typeof (event.id) !== 'undefined') {
               element.qtip({
                  ...
                  position: {
                     my: 'bottom left',
                     at: 'top left',
                     // View Tools start #42595
                     adjust: {
                        y: 8,
                     },
                     // View Tools end #42595
                  },
                  show: {
                     solo: true
                  },
...
// mint - refresh broken calendar dashlet:
$(function () {
   var calId = window.dashlet_guid ? window.dashlet_guid.substr(1) : false;
   if (!calId || document.querySelector('div.slot').offsetTop > 150) {
      CAL.refresh();
   }
});
// mint;
...
$(document).ready(function () {
   if (typeof global_current_user_id != "undefined" &&
      $('#calendar' + global_current_user_id).find('.fc-view table').length < 1) {
