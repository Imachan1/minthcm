...
if ( typeof (SUGAR) == "undefined" ) {
 ...
 var inputsWithErrors = inputsWithErrors || [ ];
 ...
 function checkMinSupported( c, s ) {
    ...
    for ( var i in supported ) {
       if ( current[i] && parseInt( current[i] ) > parseInt( supported[i] ) )
          return true;
       else if ( current[i] && parseInt( current[i] ) < parseInt( supported[i] ) )
          return false;
    }
    return true;
 }
 ...
 if ( typeof date_reg_format != "string" )
    ...
    try {
       ...
       YUI().use( 'node', function ( Y ) {
          Y.one( inputHandle ).get( 'parentNode' ).get( 'children' ).each( function ( node, index, nodeList ) {
             if ( node.hasClass( 'validation-message' ) && node.get( 'text' ) == matchTxt ) { // View Tools #40728 && node.get( 'text' ).search( matchTxt ) ) {
                raiseFlag = true;
             }
          } );
       } );
 ...
 function isFieldTypeExceptFromEmptyCheck( fieldType ) {
    var results = false;
    var exemptList = [ 'bool', 'file', 'alpha' ]; /* View Tools #53237 - added 'alpha' */
 ...
 function validate_form( formname, startsWith ) {
    ...
    // clear_all_errors(); // remove previous error messages // View Tools #38123
    // View Tools start #38123
    //inputsWithErrors = new Array();
    inputsWithErrors = inputsWithErrors || [ ];
    // View Tools end #38123
    // View Tools start
    var old_isError = isError;
    // View Tools end
                      ...
                      case 'range':
                         if ( !inRange( trim( form[validate[formname][i][nameIndex]].value ), validate[formname][i][minIndex], validate[formname][i][maxIndex] ) ) {
                            ...
                            if ( typeof validate[formname][i][minIndex] == 'number' && typeof validate[formname][i][maxIndex] == 'number' ) {
                               ...
                               add_error_style( formname, validate[formname][i][nameIndex], validate[formname][i][msgIndex] + " " + lbl_validate_range + " (" + validate[formname][i][minIndex] + " - " + validate[formname][i][maxIndex] + ")" );
                               // View Tools end #37619 
 ...
 function parseDate( input, format ) {
    ...
    // 06-11-1 or 06-1-1
       ...
       digits = 0;
       if ( date.match( /^\d{8}$/ ) )
          digits = 8;// match for 8 digits
       else if ( date.match( /\d{6}/ ) )
          digits = 6;// match for 5 digits
       else if ( date.match( /\d{4}/ ) )
          digits = 4;// match for 5 digits
       else if ( date.match( /\d{5}/ ) )
          digits = 5;// match for 5 digits
 ...
 sugarListView.prototype.send_form = function ( select, currentModule, action, no_record_txt, action_module, return_info, ajax, callback ) {
    if ( document.MassUpdate.select_entire_list.value == 1 ) {
       ...
       if ( action.indexOf( 'entryPoint=formLetter' ) != -1 ) {
 ...
 sugarListView.get_checks = function () {
    ...
    if ( typeof document.MassUpdate != 'undefined' ) {
       ...
       if ( uids.length == 0 )
          return false; // return false if no checks to get
 ...
 function formatNumber( n, num_grp_sep, dec_sep, round, precision ) {
    ...
    if ( n.length > 2 )
       return n.join( '.' ); // that's not a num!
    // round
    ...
    if ( typeof precision != 'undefined' && precision >= 0 ) {
       if ( n.length > 1 && typeof n[1] != 'undefined' )
          n[1] = n[1].substring( 0, precision ); // cut off precision
       else
          n[1] = '';
 ...
 SUGAR.unifiedSearchAdvanced = function () {
    ...
    return {
       init: function () {
          ...
          if ( !SUGAR.unifiedSearchAdvanced.usa_div || !SUGAR.unifiedSearchAdvanced.usa_img )
             return;
 ...
 if ( typeof YAHOO != 'undefined' )
    YAHOO.util.Event.addListener( window, 'load', SUGAR.unifiedSearchAdvanced.init );
 ...
 SUGAR.util = function () {
    ...
    return {
          ...
          if ( UA.webkit && !UA.chrome ) {
             var waitUntilLoaded = function () {
                ...
                if ( SUGAR.evalScript_waitCount == 0 ) {
                   ...
                   SUGAR.evalScript_evalElem = [ ]; //viewTools #36866
                }
             };
             ...
             SUGAR.evalScript_evalElem = (typeof SUGAR.evalScript_evalElem == "object") ? SUGAR.evalScript_evalElem : new Array(); //viewTools #36866
       ...
       clearAdditionalDetailsCall: function () {
          if ( typeof SUGAR.util.additionalDetailsRpcCall == 'number' )
             window.clearTimeout( SUGAR.util.additionalDetailsRpcCall );
       },
       /**
 ...
 SUGAR.savedViews = function () {
    ...
    return {
       setChooser: function () {
          ...
          if ( typeof left_td == 'undefined' || left_td == null )
             return; // abort!
       ...
       handleForm: function () {
          ...
          if ( SUGAR.savedViews.selectedSortOrder == 'DESC' )
             ...
             document.getElementById( 'sort_order_asc_radio' ).checked = true;
       }
    };
 }();
          ...
          if ( document.getElementById( module + theView + 'SearchForm' ).innerHTML == '' ) {
             ...
             var success = function ( data ) {
                ...
                if ( theView == 'saved_views' ) {
                   if ( typeof columnsMeta != 'undefined' )
                      SUGAR.savedViews.columnsMeta = columnsMeta;
                   if ( typeof selectedOrderBy != 'undefined' )
                      SUGAR.savedViews.selectedOrderBy = selectedOrderBy;
                   if ( typeof selectedSortOrder != 'undefined' )
 ...
 SUGAR.contextMenu.actions = function () {
    return {
       ...
       createNote: function ( itemClicked, metaData ) {
          ...
          for ( i in metaData ) {
             if ( i == 'notes_parent_type' )
                loc += '&parent_type=' + metaData[i];
             else if ( i != 'module' && i != 'parent_type' )
                loc += '&' + i + '=' + metaData[i];
          }
 ...
 //               var selectField = window.document.forms[form_name].elements[the_key];
             if ( window.document.forms[form_name].elements[the_key].length > 1 ) {
                var selectField = window.document.forms[form_name].elements[the_key + "_select"];
                if ( selectField != undefined ) {
 // View Tools #43197 END
                   for ( var i = 0; i < selectField.options.length; i++ ) {
                      if ( selectField.options[i].text == displayValue ) {
                         selectField.options[i].selected = true;
 // View Tools #43197 START
                         window.document.getElementById( the_key ).value = selectField.options[i].value;
                         SUGAR.util.callOnChangeListers( window.document.getElementById( the_key ) );
 // View Tools #43197 END
 ...
 if ( typeof (SUGAR.AutoComplete) == 'undefined' )
    SUGAR.AutoComplete = {};
