...
request_id++;checkRefreshPage(url);}});}else{var subpanel=document.getElementById('subpanel_'+child_field);subpanel.style.display='';set_div_cookie(subpanel.cookie_name,'');current_child_field=child_field;checkRefreshPage(url);}}
function toggleSubpanelCookie(tab){set_div_cookie(get_module_name()+'_'+tab+'_v',!$('#subpanel_'+tab).is(":visible"));}
function hideQCSubpanelForm(){let quickCreateDiv=YAHOO.util.Selector.query("div.quickcreate",null,true);if(quickCreateDiv){let form=YAHOO.util.Selector.query("form",quickCreateDiv,true);if(form){let moduleName=YAHOO.util.Selector.query('input[name=module]',form,true).value;let cancelButtonName=moduleName+"_subpanel_cancel_button";isVisible=$('#'+cancelButtonName).is(":visible");if(isVisible){SUGAR.subpanelUtils.cancelCreate(cancelButtonName,function(){cancelled=true;});}}}}
function toggleSubpanelCookie(tab){set_div_cookie(get_module_name()+'_'+tab+'_v',!$('#subpanel_'+tab).is(":visible"),hideQCSubpanelForm());}
...
viewTools.GUI.statusBox.showStatus(viewTools.language.get('app_strings','LBL_SAVING')+'...','info');var success=function(data){var module=get_module_name();var id=get_record_id();var layout_def_key=get_layout_def_key();try{SUGAR.util.globalEval('result = '+data.responseText);}catch(err){}
...
viewTools.GUI.statusBox.showStatus(viewTools.language.get('app_strings','LBL_SAVED'),'success',1000);$("#bootstrap-container .mask").remove();for(var i=0;i<saveButton.length;i++){saveButton[i].disabled=false;}}}
