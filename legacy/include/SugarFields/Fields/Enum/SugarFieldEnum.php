...
class SugarFieldEnum extends SugarFieldBase {
   ...
   function displayFromFunc($displayType, $parentFieldArray, $vardef, $displayParams, $tabindex = 0) {
      ...
      if ( !empty($vardef['function']) && $displayType == 'ListView' ) {
         if ( is_array($vardef['function']) && !empty($vardef['function']['name']) ) {
            if ( !empty($vardef['function']['include']) ) {
               require_once($vardef['function']['include']);
            }
            $function_name = $vardef['function']['name'];
         } else {
            $function_name = $vardef['function'];
         }
         $vardef['options_list'] = $function_name($parentFieldArray, strtolower($vardef['name']), $parentFieldArray[strtoupper($vardef['name'])], $displayType);
      }
