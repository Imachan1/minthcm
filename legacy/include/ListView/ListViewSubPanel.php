...
if (!defined('sugarEntry') || !sugarEntry) {
    ...
    class ListViewSubPanel extends ListView
    {
        ...
        public function process_dynamic_listview($source_module, $sugarbean, $subpanel_def, $countOnly = false)
        {
            ...
            $this->clear_query = $list_data['query']; //viewTools #36866 #40449
        ...
        public function process_dynamic_listview_rows($data, $parent_data, $smartyTemplateSection, $html_varName, $subpanel_def)
        {
            ...
            global $sugar_config;
            if ( !isset($sugar_config['subpanel_count_method']) || $sugar_config['subpanel_count_method'] != 'not_count' ) {
                global $db;
                $count_query = "SELECT count(*) count FROM (" . $this->clear_query;
                if ( isset($sugar_config['subpanel_count_method']) && $sugar_config['subpanel_count_method'] == 'limited_count' ) {
                        ...
                        if ($usage != 'query_only') {
                            ...
                            if (isset($aItem->field_defs[strtolower($list_field['name'])])) {
                                ...
                                if (isset($list_field['widget_class']) && $list_field['widget_class'] == "SubPanelIcon") {
                                    ...
                                    if ((($list_field['name'] === 'edit_button' && $field_acl['EditView']) || ($list_field['name'] === 'close_button' && $field_acl['EditView']) || ($list_field['name'] === 'remove_button' && ($field_acl['EditView'] || ($aItem->object_name=="SecurityGroup" && $subpanel_def->parent_bean->ACLAccess('edit'))) )) && '' != ($_content = $layout_manager->widgetDisplay($list_field))) {
