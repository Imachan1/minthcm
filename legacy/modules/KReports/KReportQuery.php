...
/**
 * Mint #59708: getOwnerWhere was not respected
 */
/* * *******************************************************************************
...
class KReportQueryArray
{
    ...
    public function __construct($rootModule = '', $whereOverride = array(), $unionModules = '', $evalSQLFunctions = true, $listFields = array(), $unionListFields = array(), $whereFields = array(), $additonalFilter = '', $whereGroupFields = array(), $additionalGroupBy = array(), $addParams = array())
    {
    ...
    public function context_to_array($contextString)
    {
    ...
    public function handle_where_conditions()
    {
        ...
        foreach ($this->whereArray as $originalKey => $originalData) {
            ...
            if ('reference' == $originalData['operator']) {
                $referencingFields[$originalKey] = $originalData['value'];
            }

            if ('parent_assign' == $originalData['operator']) {
                ...
                foreach ($this->whereArray as $originalKey => $originalData) {
                    if ($originalData['fieldid'] == $overrideData['fieldid']) {
                        ...
                        foreach ($transferFields as $thisFieldName) {
                            // Mint start #58178
                            if (isset($overrideData[$thisFieldName])) {
        ...
        foreach ($this->whereArray as $whereId => $whereArrayEntry) {
            ...
            if ('datetime' == $whereArrayEntry['type']) {
            ...
            if ('yo2' == $whereArrayEntry['usereditable']) {
                unset($this->whereArray[$whereId]);
            }
            //['operator'] = 'ignore';
// 2011-03-25 added function to be evaluated
            if ('function' == $whereArrayEntry['operator']) {
                include 'modules/KReports/kreportsConfig.php';
                //$customFunctionInclude = 'modules/KReports/kreportsConfig.php'; //Mint #48885 it is not correct line. $customFunctionInclude is in config file.
        ...
        foreach ($this->whereArray as $whereId => $whereArrayEntry) {
            ...
            if ('ignore' == $whereArrayEntry['operator']) {
                unset($this->whereArray[$whereId]);
            }
        }
    ...
    public function get_where_array()
    {
        ...
        foreach ($this->whereArray as $thisWhereCondition) {
            ...
            if ('yes' == $thisWhereCondition['exportpdf'] && 'ignore' != $thisWhereCondition['operator']) {
    ...
    public function build_query_strings()
    {
        ...
        if ('' != $this->union_modules) {
            ...
            while ($i < count($this->whereArray)) {
                if ('root' == $this->whereArray[$i]['unionid']) {
            ...
            while ($i < count($this->whereGroupsArray)) {
                if ('root' == $this->whereGroupsArray[$i]['unionid']) {
            ...
            // Mint start #58178
            $whereArray = isset($this->queryArray['root']['whereArray']) ? $this->queryArray['root']['whereArray'] : array();
            $whereGroupsArray = isset($this->queryArray['root']['whereGroupsArray']) ? $this->queryArray['root']['whereGroupsArray'] : array();
            $this->queryArray['root']['kQuery'] = new KReportQuery($this->root_module, $this->evalSQLFunctions, $this->listArray, $whereArray, $this->whereAddtionalFilter, $whereGroupsArray, $this->additionalGroupBy, $this->addParams);
            // Mint end #58178
    ...
    public function buildTotalSelectString($fullQuery, $valueBase = 'valuetype')
    {
...
class KReportQuery
{
    ...
    public function build_query_strings()
    {
        if ('mssql' == $GLOBALS['db']->dbType || 'oci8' == $GLOBALS['db']->dbType) {
    ...
    public function switchJoinType($jointype)
    {
            ...
            foreach ($this->whereArray as $thisWhere) {
                ...
                if (is_array($this->whereGroupsArray)) {
                    ...
                    foreach ($this->whereGroupsArray as $thisWhereGroupEntry) {
                        if ($thisWhereGroupEntry['id'] == $thisWhere['groupid'] && array_key_exists('notexists', $thisWhereGroupEntry) && '1' == $thisWhereGroupEntry['notexists']) {
                            $flagNotExists = false;
                        }
    ...
    public function addPath($path, $jointype)
    {
        if ('' != $path) {
            ...
            if (!isset($this->tablePath[$path])) {
                ...
                if ('' == $this->tablePath[$path] || ('LEFT JOIN' == $this->tablePath[$path] && 'INNER JOIN' == $jointype)) {
                    $this->tablePath[$path] = $jointype;
                }
    ...
    public function build_from_string()
    {
        ...
        if ('none' != $this->authChecklevel) {
            // Mint start #58178
            $authCheckType = isset($GLOBALS['sugar_config']['KReports']['authCheck']) ? $GLOBALS['sugar_config']['KReports']['authCheck'] : '';
            ...
            while ($levelCounter <= $this->maxDepth) {
                ...
                foreach ($this->joinSegments as $thisPath => $thisPathDetails) {
                    ...
                    if ($thisPathDetails['level'] == $levelCounter) {
                        ...
                        elseif ('relationship' == $rightArray[0]) {
                        ...
                        elseif ('relate' == $rightArray[0]) {
                            ...
                            if ('' != $thisPathDetails['jointype']) {
                                ...
                                if (isset($this->joinSegments[$leftPath]['object']->field_defs[$rightArray[2]]['side']) && 'left' == $this->joinSegments[$leftPath]['object']->field_defs[$rightArray[2]]['side'] && !$this->joinSegments[$leftPath]['object']->$rightArray[2]->_swap_sides) {
                                    $this->joinSegments[$leftPath]['object']->$rightArrayEl->_swap_sides = true;
                                }
                                //$rightArray[2]
                            ...
                            // Mint start #58178
                            if ('none' != $this->authChecklevel && 'top' != $this->authChecklevel && isset($GLOBALS['sugar_config']['KReports'])) {
                                // Mint end #58178
    ...
    public function build_select_string($unionJoinSegments = '')
    {
        // require_once('include/utils.php');
        ...
        // build select
        if ($this->isGrouped && ('mssql' == $GLOBALS['db']->dbType || 'oci8' == $GLOBALS['db']->dbType)) {
        ...
        if (is_array($this->listArray)) {
            foreach ($this->listArray as $thisListEntry) {
                ...
                if ('' == $thisListEntry['path'] || (isset($thisListEntry['fixedvalue']) && '' != $thisListEntry['fixedvalue'])) {
                    ...
                    if ('-' != $thisListEntry['sqlfunction'] && $this->evalSQLFunctions && ('kreporter' != $this->joinSegments[$pathName]['object']->field_defs[$fieldArray[1]]['type'] || ('kreporter' == $this->joinSegments[$pathName]['object']->field_defs[$fieldArray[1]]['type'] && 'X' == $this->joinSegments[$pathName]['object']->field_defs[$fieldArray[1]]['evalSQLFunction']))) {
                        if ('GROUP_CONCAT' == $thisListEntry['sqlfunction']) {
                        ...
                        // }
                            ...
                            //2013-04-22 also for Count Distinct ... Bug #469
                            $this->unionSelectString .= ', ' . ('COUNT' == $thisListEntry['sqlfunction'] || 'COUNT_DISTINCT' == $thisListEntry['sqlfunction'] ? 'SUM' : $thisListEntry['sqlfunction']) . '(' . $thisListEntry['fieldid'] . ')';
                        }
                    ...
                    $this->fieldNameMap[$thisListEntry['fieldid']] = array(
                        'fieldname' => '',
                        'path' => '',
                        'islink' => ('yes' == $thisListEntry['link']) ? true : false,
                        'sqlFunction' => '',
                    ...
                    if ('-' != $thisListEntry['sqlfunction'] && $this->evalSQLFunctions && ('kreporter' != $this->joinSegments[$pathName]['object']->field_defs[$fieldArray[1]]['type'] || ('kreporter' == $this->joinSegments[$pathName]['object']->field_defs[$fieldArray[1]]['type'] && (array_key_exists('evalSQLFunction', $this->joinSegments[$pathName]['object']->field_defs[$fieldArray[1]]) && 'X' == $this->joinSegments[$pathName]['object']->field_defs[$fieldArray[1]]['evalSQLFunction'])))) {
                        if ('GROUP_CONCAT' == $thisListEntry['sqlfunction']) {
                            $this->selectString .= ', ' . $thisListEntry['sqlfunction'] . '(DISTINCT ' . $this->get_field_name($pathName, $fieldArray[1], $thisListEntry['fieldid'], ('yes' == $thisListEntry['link']) ? true : false, $thisListEntry['sqlfunction']) . ' SEPARATOR \', \')';
                            $this->unionSelectString .= ', ' . $thisListEntry['sqlfunction'] . '(DISTINCT ' . $thisListEntry['fieldid'] . ' SEPARATOR \', \')';
                        } //2013-03-01 Sort function for Group Concat
                        elseif ('GROUP_CONASC' == $thisListEntry['sqlfunction']) {
                            $this->selectString .= ', GROUP_CONCAT(DISTINCT ' . $this->get_field_name($pathName, $fieldArray[1], $thisListEntry['fieldid'], ('yes' == $thisListEntry['link']) ? true : false, $thisListEntry['sqlfunction']) . ' ORDER BY ' . $this->get_field_name($pathName, $fieldArray[1], $thisListEntry['fieldid'], ('yes' == $thisListEntry['link']) ? true : false, $thisListEntry['sqlfunction']) . ' ASC SEPARATOR \', \')';
                            $this->unionSelectString .= ', GROUP_CONCAT(DISTINCT ' . $thisListEntry['fieldid'] . ' ORDER BY ' . $thisListEntry['fieldid'] . ' ASC SEPARATOR \', \')';
                        } elseif ('GROUP_CONDSC' == $thisListEntry['sqlfunction']) {
                            $this->selectString .= ', GROUP_CONCAT(DISTINCT ' . $this->get_field_name($pathName, $fieldArray[1], $thisListEntry['fieldid'], ('yes' == $thisListEntry['link']) ? true : false, $thisListEntry['sqlfunction']) . ' ORDER BY ' . $this->get_field_name($pathName, $fieldArray[1], $thisListEntry['fieldid'], ('yes' == $thisListEntry['link']) ? true : false, $thisListEntry['sqlfunction']) . ' ASC SEPARATOR \', \')';
                            $this->unionSelectString .= ', GROUP_CONCAT(DISTINCT ' . $thisListEntry['fieldid'] . ' ORDER BY ' . $thisListEntry['fieldid'] . ' DESC SEPARATOR \', \')';
                        } // 2012-10-11 add count distinct
                        elseif ('COUNT_DISTINCT' == $thisListEntry['sqlfunction']) {
                            $this->selectString .= ', COUNT(DISTINCT ' . $this->get_field_name($pathName, $fieldArray[1], $thisListEntry['fieldid'], ('yes' == $thisListEntry['link']) ? true : false, $thisListEntry['sqlfunction']) . ')';
                            //2013-04-22 also for Count Distinct ... Bug #469
                            //$this->unionSelectString .= ', COUNT(DISTINCT ' . $thisListEntry['fieldid'] . ')';
                            $this->unionSelectString .= ', SUM(' . $thisListEntry['fieldid'] . ')';
                        } else {
                            $this->selectString .= ', ' . $thisListEntry['sqlfunction'] . '(' . $this->get_field_name($pathName, $fieldArray[1], $thisListEntry['fieldid'], ('yes' == $thisListEntry['link']) ? true : false, $thisListEntry['sqlfunction']) . ')';
                            // 2011-05-31 if function is count - sum in union
                            $this->unionSelectString .= ', ' . ('COUNT' == $thisListEntry['sqlfunction'] ? 'SUM' : $thisListEntry['sqlfunction']) . '(' . $thisListEntry['fieldid'] . ')';
                        }
                    } else {
                        ...
                        //else
                        $this->selectString .= ', ' . $this->get_field_name($pathName, $fieldArray[1], $thisListEntry['fieldid'], ('yes' == $thisListEntry['link']) ? true : false);
                        $this->unionSelectString .= ', ' . $thisListEntry['fieldid'];
                    }
                ...
                if (isset($this->joinSegments[$pathName]) && ('currency' == $this->joinSegments[$pathName]['object']->field_defs[$fieldArray[1]]['type'] || (isset($this->joinSegments[$pathName]['object']->field_defs[$fieldArray[1]]['kreporttype']) && 'currency' == $this->joinSegments[$pathName]['object']->field_defs[$fieldArray[1]]['kreporttype'])) || 'currency' == $this->rootfieldNameMap[$thisListEntry['fieldid']]['type']) {
    ...
    public function build_where_string()
    {
        global $db, $app_list_strings, $beanList, $beanFiles, $current_user;
        ...
        foreach ($this->whereGroupsArray as $whereGroupIndex => $thisWhereGroup) {
            ...
            foreach ($this->whereArray as $thisWhere) {
                ...
                if (($thisWhere['groupid'] == $thisWhereGroup['groupid'] || $thisWhere['groupid'] == $thisWhereGroup['id']) && 'ignore' != $thisWhere['operator']) {
                    ...
                    if ('notexisting' != $thisWhere['jointype']) {
            ...
            // Mint start #58178
            $groupIndex = isset($thisWhereGroup['group']) ? $thisWhereGroup['group'] : $thisWhereGroup['groupid'];
            $arrayWhereGroupsIndexed[$groupIndex] = $thisWhereGroup;
            // Mint end #58178
        }
    ...
    public function buildWhereClauseForLevel($thisLevel, $completeArray = array())
    {
        $whereClause = '';
    ...
    public function buildWhereClauseForChildren($thisChildren, $thisOperator, $completeArray)
    {
        $whereClause = '';
    ...
    public function getWhereOperatorClause($operator, $fieldname, $fieldid, $path, $value, $valuekey, $valueto, $valuetokey = '', $jointype = '')
    {
        ...
        if (isset($valuekey) && '' != $valuekey && 'undefined' != $valuekey) {
            $value = $valuekey;
        }

        if (isset($valuetokey) && '' != $valuetokey && 'undefined' != $valuetokey) {
            $valueto = $valuetokey;
        }
        ...
        if ('lastndays' != $operator && 'lastnfdays' != $operator && 'lastnweeks' != $operator && 'notlastnweeks' != $operator && 'lastnfweeks' != $operator && 'nextndays' != $operator && 'nextnweeks' != $operator && 'notnextnweeks' != $operator && 'betwndays' != $operator &&
            ('lastnddays' != $operator && !is_numeric($value)) &&
            ('nextnddays' != $operator && !is_numeric($value)) &&
            ('betwnddays' != $operator && !is_numeric($value))
        ) {
            // Mint start #58178
            if (isset($this->fieldNameMap[$fieldid]) && 'date' === $this->fieldNameMap[$fieldid]['type']) {
                // Mint end #58178
            ...
            // Mint start #58178
            if (isset($this->fieldNameMap[$fieldid]) && ('datetime' === $this->fieldNameMap[$fieldid]['type'] || 'datetimecombo' === $this->fieldNameMap[$fieldid]['type'])) {
                // Mint end #58178
        ...
        switch ($operator) {
            ...
            case 'between':
                ...
                if ('date' == $this->fieldNameMap[$fieldid]['type'] || 'datetime' == $this->fieldNameMap[$fieldid]['type'] || 'datetimecombo' == $this->fieldNameMap[$fieldid]['type'])
            ...
            case 'isnotempty':
                ...
                break;
            case 'oneof':
                if ('multienum' == $this->fieldNameMap[$fieldid]['type']) {
            ...
            case 'firstdaynextmonth':
                $dateArray = getdate();
                $fromDate = date('Y-m-d', mktime(0, 0, 0, '12' == $dateArray['mon'] ? 1 : $dateArray['mon'] + 1, 1, '12' == $dateArray['mon'] ? $dateArray['year'] + 1 : $dateArray['year']));
                $thisWhereString .= ' >= \'' . $fromDate . ' 00:00:00\' AND ' . $this->get_field_name($path, $fieldname, $fieldid) . ' <= \'' . $fromDate . ' 23:59:59\'';
                break;
            case 'nthdayofmonth':
                $dateArray = getdate();
        ...
        if ('optional' == $jointype && 'isnull' != $operator) {
    ...
    public function check_groupby($additionalGroupBy = array())
    {
        ...
        if ($this->isGrouped) {
            foreach ($this->listArray as $listArrayIndex => $thisList) {
                if ('no' == $thisList['groupby'] && ('mssql' == $GLOBALS['db']->dbType || 'oci8' == $GLOBALS['db']->dbType) && $this->evalSQLFunctions && ('' == $thisList['sqlfunction'] || '-' == $thisList['sqlfunction'])) {
    ...
    public function build_groupby_string($additionalGroupBy = array())
    {
        global $db, $app_list_strings, $beanList, $beanFiles;
        ...
        if (is_array($this->listArray)) {
            ...
            foreach ($this->listArray as $thisList) {
                ...
                if ((!$this->exclusiveGroupinbgByAddParams && 'no' != $thisList['groupby']) || in_array($thisList['fieldid'], $additionalGroupBy)) {
    ...
    public function build_orderby_string()
    {
        global $db, $app_list_strings, $beanList, $beanFiles;
        ...
        if (is_array($this->listArray)) {
            ...
            if (count($this->sortOverride) > 0) {
                if ($this->orderByFieldID) {
                    ...
                    } elseif ('COUNT_DISTINCT' == $this->fieldNameMap[$this->sortOverride['sortid']]['sqlFunction']) {
                ...
                foreach ($this->listArray as $thisList) {
                    if ('asc' == $thisList['sort'] || 'desc' == $thisList['sort']) {
                        ...
                        if ('' != $thisList['sortpriority']) {
                            ...
                            if ($this->orderByFieldID) {
                                ...
                                } elseif ('COUNT_DISTINCT' == $thisList['sqlfunction']) {
    ...
    public function get_table_for_module($module)
    {
    ...
    public function get_field_name($path, $field, $fieldid, $link = false, $sqlFunction = '')
    {
        // if we do not have a path we have a fixed value field so do not return a name
        if ('' != $path) {
            // normal processing
            $thisAlias = (isset($this->joinSegments[$path]['object']->field_defs[$field]['source']) && 'custom_fields' == $this->joinSegments[$path]['object']->field_defs[$field]['source']) ? $this->joinSegments[$path]['customjoin'] : $this->joinSegments[$path]['alias'];
            ...
            if (!isset($this->fieldNameMap[$fieldid])) {
                // Mint start #58178
                $fieldData = $this->joinSegments[$path]['object']->field_defs[$field];
                $fieldType = isset($fieldData['kreporttype']) ? $fieldData['kreporttype'] : $fieldData['type'];
                // Mint end #58178
            ...
            if ('kreporter' == $this->joinSegments[$path]['object']->field_defs[$field]['type']) {
                ...
                // Mint start #58178
                $idEntryGrouping = isset($thisFieldIdEntry['grouping']) ? $thisFieldIdEntry['grouping'] : '';
                return $this->wrap_field_grouping($thisAlias, $field, $idEntryGrouping);
                // Mint end #58178
            }
        } else {
            return $fieldid;
    ...
    public function get_fieldname_by_fieldid($fieldid)
    {
        return isset($this->fieldNameMap[$fieldid]) ? $this->fieldNameMap[$fieldid]['fieldname'] : '';
    }

    public function get_fieldpath_by_fieldid($fieldid)
    {
        return isset($this->fieldNameMap[$fieldid]) ? $this->fieldNameMap[$fieldid]['path'] : '';
    }

    public function get_listfieldentry_by_fieldid($fieldid)
    {
