...
class SpentTime extends Basic
{
    ...
    public $employee_id;
    public $spent_time;
    public $spendtime_projecttask_id;
    public $spendtime_project_id;
    public $work_date;
    public $workschedule_id;
    public $date_start;
    public $date_end;
    ...
    public function save($check_notify = false)
    {
        ...
        if (!$other_worktime_count) {
            ...
            if (isset($_REQUEST['action'])) {
                if ('DeleteRelationship' == $_REQUEST['action']) {
    ...
    public function ACLAccess($view, $is_owner = 'not_set', $in_group = 'not_set')
    {
        ...
        if (is_admin($current_user) || is_admin_for_module($current_user, $this->getACLCategory())) {
            ...
            if (in_array($_view, array('edit', 'editview', 'save', 'massupdate', 'delete')) && false !== $result && !empty($this->workschedule_id)) {
    ...
    {
        if ($this->mint_is_update) {
            $this->auditBean($this->mint_is_update);
        }
