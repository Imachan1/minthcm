...
class Currency extends SugarBean {
   ...
   public function retrieve($id = -99, $encode = true, $deleted = true) 
   {
      if ( $id == '-99' ) {
         ...
         // View Tools start #40674
         $this->currency_on_right = $this->getDefaultCurrencyOnRight();
         $this->hidden = 0;
         // View Tools end #40674
         $this->deleted = 0;
         $this->status = 'Active';
...
function format_place_symbol($amount, $symbol, $symbol_space, $currency = null) { // View Tools #40674 ($currency)
   if ( $symbol != '' ) {
      if ( $symbol_space == true ) {
         // View Tools start #40674
         if ( isset($currency) && $currency->currency_on_right ) {
