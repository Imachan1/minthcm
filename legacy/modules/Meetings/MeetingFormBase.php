      ...
      if ( (isset($_POST['isSaveFromDetailView']) && $_POST['isSaveFromDetailView'] == 'true') ||
         ...
         if ( $focus->status == 'Held' && $this->isEmptyReturnModuleAndAction() && !$this->isSaveFromDCMenu() ) {
            ...
            if ( !empty($_POST['candidate_invitees']) ) {
               ...
               $candidateInvitees = array();
            }

            $deleteCandidates = array();
            $focus->load_relationship('candidates');
            $q = 'SELECT mu.candidate_id, mu.accept_status FROM meetings_candidates mu WHERE mu.meeting_id = \'' . $focus->id . '\' AND mu.deleted=0';
            $r = $focus->db->query($q);
            $acceptStatusCandidates = array();
            while ( $a = $focus->db->fetchByAssoc($r) ) {
               if ( !in_array($a['candidate_id'], $candidateInvitees) ) {
                  $deleteCandidates[$a['candidate_id']] = $a['candidate_id'];
               } else {
                  $acceptStatusCandidates[$a['candidate_id']] = $a['accept_status'];
               }
            }

            if ( count($deleteCandidates) > 0 ) {
               foreach ( $deleteCandidates as $u ) {
            ...
            if ( !empty($_POST['resource_invitees']) ) {
               ...
               $resourceInvitees = array();
            }

            $deleteResources = array();
            $focus->load_relationship('resources');
            $q = 'SELECT mu.resource_id FROM meetings_resources mu WHERE mu.meeting_id = \'' . $focus->id . '\' AND mu.deleted=0';
            $r = $focus->db->query($q);
            $acceptStatusResources = array();
            while ( $a = $focus->db->fetchByAssoc($r) ) {
               if ( !in_array($a['resource_id'], $resourceInvitees) ) {
                  $deleteResources[$a['resource_id']] = $a['resource_id'];
               } else {
                  $acceptStatusResources[$a['resource_id']] = $a['resource_id'];
               }
            }

            if ( count($deleteResources) > 0 ) {
               foreach ( $deleteResources as $u ) {
            ...
            if ( !empty($_POST['existing_candidate_invitees']) ) {
               $existing_candidates = explode(",", trim($_POST['existing_candidate_invitees'], ','));
            }

            foreach ( $focus->candidates_arr as $candidate_id ) {
               if ( empty($candidate_id) || isset($existing_candidates[$candidate_id]) || isset($deleteCandidates[$candidate_id]) ) {
                  continue;
               }

               if ( !isset($acceptStatusCandidates[$candidate_id]) ) {
            ...
            if ( !empty($_POST['existing_resource_invitees']) ) {
               $existing_resources = explode(",", trim($_POST['existing_resource_invitees'], ','));
            }

            foreach ( $focus->resources_arr as $resource_id ) {
               if ( empty($resource_id) || isset($existing_resources[$resource_id]) || isset($deleteResources[$resource_id]) ) {
                  continue;
               }

               if ( !isset($acceptStatusResources[$resource_id]) ) {
