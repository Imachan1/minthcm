...
html+='<tr class="'+className+'">';html+='<td><span class="suitepicon suitepicon-module-'+bean.module.toLowerCase().replace('_','-')+'"></span></td>';if(bean.module=='Resources'||bean.module=="SecurityGroup"){html+='<td>'+bean.fields.name+'</td><td></td><td></td>';}else{html+='<td>'+bean.fields.full_name+'</td>';html+='<td>'+bean.fields.email1+'</td>';if(bean.module=='Candidates'){html+='<td>'+bean.fields.phone_mobile+'</td>';}else{html+='<td>'+bean.fields.phone_work+'</td>';}}
html+='<td align="right">';hidden='visible';if(!disabled){}
...
SugarWidgetSchedulerAttendees.get_users=function(list_row){if(typeof(GLOBAL_REGISTRY.result_list[list_row])!='undefined'){viewTools.api.callCustomApi({module:'Meetings',action:'getUsers',dataPOST:{id:GLOBAL_REGISTRY.result_list[list_row].fields.id},callback:function(data){data.forEach((person)=>{if(!SugarWidgetSchedulerAttendees.userOnList(person.id)){item={fields:person,module:"User"}
GLOBAL_REGISTRY.focus.users_arr.push(item)}});}});}}
SugarWidgetSchedulerAttendees.userOnList=function(id){var user_on_list=false;for(var i=0;i<GLOBAL_REGISTRY.focus.users_arr.length;i++){if(GLOBAL_REGISTRY.focus.users_arr[i].fields.id==id){user_on_list=true;break;}}
return user_on_list;}
...
var query={"modules":["Users","Candidates","Resources","SecurityGroups"],"field_list":['id','name','full_name','email1','phone_work','phone_mobile','show_on_employees','group_type'],"group":"and","conditions":conditions};global_request_registry[req_count]=[this,'display'];req_id=global_rpcClient.call_method('query',query);global_request_registry[req_id]=[GLOBAL_REGISTRY['widget_element_map'][form.id],'refresh_list'];}
...
else if(GLOBAL_REGISTRY.focus.users_arr[i].module=='Candidates'){form.candidate_invitees.value+=GLOBAL_REGISTRY.focus.users_arr[i].fields.id+",";}else if(GLOBAL_REGISTRY.focus.users_arr[i].module=='Resources'){form.resource_invitees.value+=GLOBAL_REGISTRY.focus.users_arr[i].fields.id+",";}}}
...
SugarWidgetScheduler.prototype.display=function(){this.parentNode.innerHTML='';var attendees=new SugarWidgetSchedulerAttendees();attendees.load(this.parentNode);var search=new SugarWidgetSchedulerSearch();search.load(this.parentNode);$('div#scheduler').append('<div id="SugarWidgetSchedulerPopup"></div>');$('div#SugarWidgetSchedulerPopup').hover(function(e){SugarWidgetScheduler.isMouseOverToolTip=true;},function(e){SugarWidgetScheduler.isMouseOverToolTip=false;SugarWidgetScheduler.popupControl.hide();});}
SugarWidgetScheduler.sortByStartdate=function(a,b){var dateA=new Date($('<div></div>').append(a).find('span[data-field=DATE_START]').attr('data-date'));var dateB=new Date($('<div></div>').append(b).find('span[data-field=DATE_START]').attr('data-date'));if(dateA<dateB){return-1;}else if(dateA>dateB){return 1;}
return 0;}
SugarWidgetScheduler.sortByType=function(a,b){var valueA=$('<div></div>').append(a).find('input[id=type]').attr('value');var valueB=$('<div></div>').append(b).find('input[id=type]').attr('value');if(valueA==valueB){return 0;}else if(valueB=='Meeting'){return 1;}
return 0;}
...
SugarWidgetScheduler.getScheduleDetails=function(beans,ids){var elementId='#SugarWidgetSchedulerPopup';var show_buttons=true;var caption='';var body=new Array();var width=300;var theme='';var $dialog=SugarWidgetScheduler.createDialog(elementId,body,caption,width,theme);if(_.contains(beans,'Reservationss')){viewTools.api.callCustomApi({module:'Reservations',action:'getReservations',dataPOST:{reservations_ids:ids},callback:function(data){if(!_.isEmpty(data)){var innerHTML="";var reservationTemplate=_.template('<div><div><a href="index.php?module=Reservations&action=DetailView&record=<%= id %>"><%= name %></a></div><div><%= starting_date %></div><div><%= employee %></div></div><br />');data.forEach(function(reservation){innerHTML+=reservationTemplate({id:reservation.id,name:reservation.name,starting_date:reservation.starting_date,employee:reservation.employee});});$dialog.html(innerHTML);}}});}else if(_.contains(beans,'Candidates')||_.contains(beans,'Resources')){var bean_name=$('#schedulerTable tr[data-id="'+ids[0]+'"][data-module="'+beans[0]+'"] td:first-child').text();if(bean_name){var participantTemplate=_.template('<div><h2><%= module_name %>:</h2><div><b><%= name_label %>:</b><%= name %></div></div>');$dialog.html(participantTemplate({module_name:viewTools.language.get('app_list_strings','moduleList')[beans[0]],name_label:viewTools.language.get('app_strings','LBL_NAME'),name:bean_name}));}}else{var getScheduleItems=function(){var deffereds=[];$dialog.html(SUGAR.language.get('app_strings','LBL_LOADING'));body='';jQuery.each(ids,function(index,value){var url='index.php?to_pdf=1&module=Home&action=AdditionalDetailsRetrieve&bean='+beans[index]+'&id='+ids[index]+'&show_buttons=true';deffereds.push($.ajax(url).done(function(){}).fail(function(){}).always(function(){}));});return deffereds;}
...
for(var i=0;i<GLOBAL_REGISTRY.focus.users_arr.length;i++){var row=new SugarWidgetScheduleRow(this.timeslots);row.focus_bean=GLOBAL_REGISTRY.focus.users_arr[i];GLOBAL_REGISTRY.focus.users_arr_hash[GLOBAL_REGISTRY.focus.users_arr[i]['fields']['id']]=GLOBAL_REGISTRY.focus.users_arr[i];if(GLOBAL_REGISTRY.focus.users_arr[i].module==='Resources'){row.thetableid=thetable;if(typeof(GLOBAL_REGISTRY['freebusy_adjusted'])=='undefined'||typeof(GLOBAL_REGISTRY['freebusy_adjusted'][row.focus_bean.fields.id])=='undefined'){global_request_registry[req_count]=[row,'display'];req_count++;}
viewTools.api.callCustomApi({module:'Resources',action:'getBusyTimeSlots',async:false,dataPOST:{resource_id:row.focus_bean.fields.id,timeslots:row.timeslots},callback:function(data){if(!_.isEmpty(data)){if(typeof GLOBAL_REGISTRY.freebusy_adjusted=='undefined'){GLOBAL_REGISTRY.freebusy_adjusted=new Object();}
GLOBAL_REGISTRY.freebusy_adjusted[row.focus_bean.fields.id]=data;}}});row.display();}else{row.load(thetable);}}}
SugarWidgetSchedulerAttendees.form_add_attendee=function(list_row){if(typeof(GLOBAL_REGISTRY.result_list[list_row])!='undefined'&&typeof(GLOBAL_REGISTRY.focus.users_arr_hash[GLOBAL_REGISTRY.result_list[list_row].fields.id])=='undefined'&&GLOBAL_REGISTRY.result_list[list_row].module!="SecurityGroup"){GLOBAL_REGISTRY.focus.users_arr[GLOBAL_REGISTRY.focus.users_arr.length]=GLOBAL_REGISTRY.result_list[list_row];}
if(GLOBAL_REGISTRY.result_list[list_row].module=='SecurityGroup'){SugarWidgetSchedulerAttendees.get_users(list_row);}
...
if(is_loaded){if(typeof(GLOBAL_REGISTRY['freebusy_adjusted'][this.focus_bean.fields.id][this.timeslots[i].hash])!='undefined'){var dataid='',module='';$.each(GLOBAL_REGISTRY['freebusy_adjusted'][this.focus_bean.fields.id][this.timeslots[i].hash]['records'],function(index,value){if(value.startsWith('WorkSchedules')){var ws_type=value.split('___')[1];if(ws_type!=undefined){$(td).addClass('ws-'+ws_type);}
return;}
$(td).addClass('free');if(dataid=='')
dataid=index;else
