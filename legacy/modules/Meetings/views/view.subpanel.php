...
//                                    $this->smartyTemplate->assign('CELL_COUNT', $count);
    ...
    protected function getWorkScheduleTypes($widget_contents, $parent_id) {
        ...
        FROM workschedules
        LEFT JOIN meetings ON meetings.id = '$parent_id'
        WHERE workschedules.assigned_user_id IN $user_ids
        AND (
            (workschedules.date_start >= meetings.date_start AND workschedules.date_start < meetings.date_end)
            OR
            (workschedules.date_end > meetings.date_start AND workschedules.date_end <= meetings.date_end)
            OR
