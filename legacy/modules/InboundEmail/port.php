...
class InboundEmail extends SugarBean
{
    ...
    public $hrSort = [0 => 'flagged', 1 => 'status', 2 => 'from', 3 => 'subj', 4 => 'date', ];
    public $hrSortLocal = ['flagged' => 'flagged', 'status' => 'answered', 'from' => 'fromaddr', 'subject' => 'subject', 'date' => 'senddate', ];

    public $transferEncoding = [0 => '7BIT', 1 => '8BIT', 2 => 'BINARY', 3 => 'BASE64', 4 => 'QUOTED-PRINTABLE', 5 => 'OTHER', ];
    ...
    public $required_fields = ['name' => 'name', 'server_url' => 'server_url', 'mailbox' => 'mailbox', 'user' => 'user', 'port' => 'port', ];
    ...
    public function renameFolder($oldName, $newName)
    {
        ...
        error_reporting(0);
        $imapRenameMailbox = $this->getImap()
            ->renameMailbox($oldConnect, $newConnect);
    ...
    public function checkWithPagination($offset = 0, $pageSize = 20, $order = array() , $filter = array() , $columns = array())
    {
        ...
        if (empty($filterCriteria) && SORTDATE === $sortCriteria)
        {
            // Performance fix when no filters are enabled
            $totalMsgs = $this->getImap()
                ->getNumberOfMessages();
            ...
            $emailSortedHeaders = $this->getImap()
                ->fetchOverview($sequence);

            $uids = array_map(function ($x)
            {
                return $x->uid;
            }
            , $emailSortedHeaders
        ...
        else
        {
            ...
            $emailSortedHeaders = $this->getImap()
                ->sort($sortCriteria, $sortOrder, SE_UID, $filterCriteria);
        ...
        // Get result
        $emailHeaders = $this->getImap()
            ->fetchOverview($uids, FT_UID);
        ...
        if (isset($columns['has_attachment']))
        {
            ...
            {
                $structure = $this->getImap()
                    ->fetchStructure($emailHeader['uid'], FT_UID);
    ...
    {
        if ((0 !== $imapStructure->type) && (1 !== $imapStructure->type))
        {
            return true;
        }
        ...
        {
            if (is_string($part->dparameters[0]
                ->value))
            {
    ...
    public function getFormattedRawSource($uid)
    {
        ...
        else
        {
            ...
            {
                LoggerManager::getLogger()
                    ->fatal('Inbound Email connection is not a resource for getting Formatted Raw Source');

                return null;
            }
            $raw = $this->getImap()
                ->fetchHeader($uid, FT_UID + FT_PREFETCHTEXT);
            $raw .= $this->convertToUtf8($this->getImap()
                ->getBody($uid, FT_UID));
        } // else
        $raw = to_html($raw);
        $raw = nl2br($raw);
    ...
    public function getFormattedHeaders($uid)
    {
        ...
        {
            LoggerManager::getLogger()
                ->fatal('Inbound Email connection is not a resource');

            return null;
        }

        $headers = $this->getImap()
            ->fetchHeader($uid, FT_UID);
    ...
    public function emptyTrash()
    {
        ...
        {
            LoggerManager::getLogger()
                ->warn('connection is not a valid resource to empty trush');
            $uids = array();
        }
    ...
    public function getCacheValueForUIDs($mbox, $UIDs)
    {
        ...
        $q = "SELECT * FROM email_cache WHERE ie_id = '{$this
            ->db
            ->quote($this->id) }' AND mbox = '{$this
            ->db
            ->quote($mbox) }' AND ";
        $startIndex = 0;
        $endIndex = 5;
    ...
    public function getCacheValue($mbox, $limit = 20, $page = 1, $sort = '', $direction = '')
    {
        ...
        if (!empty($this->hrSortLocal[$sort]))
        {
            $order = " ORDER BY {$this
                ->db
                ->quote($this->hrSortLocal[$sort]) } {$this
                ->db
                ->quote($direction) }";
        }
        else
    ...
    public function setCacheValue($mbox, $insert, $update = array() , $remove = array())
    {
        ...
        if (!empty($insert))
        {
            ...
            foreach ($insert as $overview)
            {
                    ...
                    ->overview->fieldDefs as $colDef)
                {
                    ...
                    if (isset($colDef['len']) && !empty($colDef['len']) && isset($colDef['type']) && !empty($colDef['type']) && 'varchar' == $colDef['type'])
                    {
                    ...
                    switch ($fieldName)
                    {
                        case "imap_uid":
                            ...
                            {
                                LoggerManager::getLogger()
                                    ->warn('Inbound email has not imap uid for setting cache value.');
                                $values .= "''";
                            }
                            else
                            {
                        ...
                        case "fromaddr":
                            $values .= $this
                                ->db
                                ->quoted($overview->from);
                        break;

                        case "message_id":
                            $values .= $this
                                ->db
                                ->quoted($overview->message_id);
                        break;
        ...
        if (!empty($update))
        {
            ...
            foreach ($update as $overview)
            {
                ...
                $q .= $set . " WHERE ie_id = '{$this
                    ->db
                    ->quote($this->id) }' AND mbox = '{$this
                    ->db
                    ->quote($overview->mbox) }' AND imap_uid = '{$overview->imap_uid}'";
    ...
    public function pop3_open()
    {
        if (!is_resource($this->pop3socket))
        {
            ...
            $socket = (isset($exServ[2]) && 'ssl' == $exServ[2]) ? "ssl://" : "tcp://";
            $socket .= $this->server_url;
            if (!$this->server_url)
            {
                LoggerManager::getLogger()
                    ->fatal('Empty host name for pop3_open');
            }
            else
            {
    ...
    public function pop3_cleanUp()
    {
        ...
        {
            LoggerManager::getLogger()
                ->fatal('POP 3 sould be a valid resource');
    ...
    public function getPop3NewMessagesToDownloadForCron()
    {
        ...
        if (null != $this->groupfolder_id && "" != $this->groupfolder_id && $this->isPop3Protocol())
        {
            ...
            if ($this->connectMailserver() == 'true')
            {
                $fetchedOverviews = $this->getImap()
                    ->fetchOverview($concatResults);
    ...
    public function pop3_checkPartialEmail($synch = false)
    {
        ...
        if (count($diff) > 0)
        {
            ...
            $fetchedOverviews = $this->getImap()
                ->fetchOverview($concatResults);
            $GLOBALS['log']->info("[EMAIL] End POP3 fetch overview on mailbox [{$this->mailbox}] for user [{$current_user->user_name}] on " . count($fetchedOverviews) . " data");
    ...
    public function getMessagesInEmailCache($msgno, $uid)
    {
        ...
        if ($this->isPop3Protocol())
        {
            ...
            {
                LoggerManager::getLogger()
                    ->fatal('Connection is not a valid resource but it is a POP3 Protocol');
            }
            else
            {
                $fetchedOverviews = $this->getImap()
        ...
        else
        {
            ...
            {
                LoggerManager::getLogger()
                    ->fatal('Connection is not a valid resource');
            }
            else
            {
                $fetchedOverviews = $this->getImap()
                    ->fetchOverview($uid, FT_UID);
            }
        } // else
        $this->updateOverviewCacheFile($fetchedOverviews);
    } // fn
    ...
    public function checkEmailOneMailbox($mailbox, $prefetch = true, $synchronize = false)
    {
        ...
        {
            LoggerManager::getLogger()
                ->warn('checkEmailOneMailbox: connection is not a valid resource');
            $searchResults = null;
        }
        else
        {
            $searchResults = $this->getImap()
                ->search($criteria, SE_UID);
        }
        ...
        if (!empty($searchResults))
        {
            ...
            $fetchedOverview = $this->getImap()
                ->fetchOverview($concatResults, FT_UID);
        ...
        if ($this->mailbox != $trashFolder)
        {
            ...
            {
                LoggerManager::getLogger()
                    ->warn('connection is not a valid resource for checkEmailOneMailbox()');
                $searchResults = null;
            }
            else
            {
    ...
    public function checkEmailOneMailboxPartial($mailbox, $prefetch = true, $synchronize = false, $start = 0, $max = - 1)
    {
        ...
        if ('done' == $ret['status'])
        {
            ...
            if ($this->mailbox != $trashFolder)
            {
                ...
                {
                    LoggerManager::getLogger()
                        ->warn('mailbox != trash folder but connection is not a valid resource for checkEmailOneMailbox()');
                    $searchResults = null;
                }
                else
                {
    ...
    public function getCachedIMAPSearch($criteria)
    {
        ...
        if (!$cacheDataExists)
        {
            ...
            {
                LoggerManager::getLogger()
                    ->fatal('Inbound Email Connection is not a valid resource.');
            }
            else
            {
                $searchResults = $this->getImap()
    ...
    public function checkEmailIMAPPartial($prefetch = true, $synch = false)
    {
        ...
        while ('done' == $ret['status'] && $index < count($mailboxes))
        {
            if ($ret['count'] > 100)
            {
    ...
    public function getMailboxProcessCount($mailbox)
    {
        ...
        {
            LoggerManager::getLogger()
                ->warn('connection is not a valid resource for getMailboxProcessCount()');
            $searchResults = null;
        }
        else
        {
            $searchResults = $this->getImap()
                ->fetchOverview($criteria, SE_UID);
        }
    ...
    public function deleteCachedMessages($uids, $fromFolder)
    {
        ...
        if (!isset($this->email) && !isset($this
            ->email
            ->et))
        {
                    ...
                    if (!empty($uid))
                    {
                        ...
                        if (!file_exists($cacheFile))
                        {
                            ...
                            $this
                                ->email
                                ->et
                                ->writeCacheFile('out', $out, $this->id, 'messages', "{$this->mailbox}{$uid}.php");
                        }
                        else
                        {
    ...
    public function markEmails($uids, $type)
    {
        ...
        {
            LoggerManager::getLogger()
                ->fatal('Inbound Email connection is not a valid resource for marking Emails');
        ...
        switch ($type)
        {
            case 'unread':
                $result = $this->getImap()
                    ->clearFlagFull($uids, '\\SEEN', ST_UID);
            break;
            case 'read':
                $result = $this->getImap()
                    ->setFlagFull($uids, '\\SEEN', ST_UID);
            break;
            case 'flagged':
                $result = $this->getImap()
                    ->setFlagFull($uids, '\\FLAGGED', ST_UID);
            break;
            case 'unflagged':
                $result = $this->getImap()
                    ->clearFlagFull($uids, '\\FLAGGED', ST_UID);
            break;
            case 'answered':
                $result = $this->getImap()
                    ->setFlagFull($uids, '\\Answered', ST_UID);
            break;
            default:
    ...
    public function deleteFolder($mbox)
    {
        ...
        {
            LoggerManager::getLogger()
                ->fatal('Inboun Email Connenction is not a valid resource for deleting Folder');
        }
        elseif ($this->getImap()
            ->unsubscribe($this->getImap()
            ->utf7Encode($connectString)))
        {
            if ($this->getImap()
                ->deleteMailbox($connectString))
            {
    ...
    public function saveNewFolder($name, $mbox)
    {
        ...
        {
            LoggerManager::getLogger()
                ->fatal('Inboun Email Connectrion is not a valid resource for saving new folder');
        }
        elseif ($this->getImap()
            ...
            ->utf7Encode($connectString)))
        {
            $this->getImap()
                ...
                ->utf7Encode($connectString));
            $status = $this->getImap()
                ->getStatus(str_replace("{$delimiter}{$name}", "", $connectString) , SA_ALL);
            ...
            foreach ($beans as $bean)
            {
                ...
                foreach ($mailboxes as $mbox)
                {
                    ...
                    if ('pop3' == $bean->protocol)
                    {
                        ...
                        $pop3Criteria .= (!empty($subject)) ? ' AND subject like "%' . $bean
                            ->db
                            ->quote($subject) . '%"' : "";
                    ...
                    else
                    {
                        ...
                        {
                            $searchOverviews = $bean
                                ->imap
                                ->fetchOverview(implode(',', $searchResult) , FT_UID);
                        } // if
    ...
    public function generateDynamicFolderQuery($type, $userId)
    {
        if (!isset($this->coreDynamicFolderQuery))
        {
            ...
            LoggerManager::getLogger()
                ->warn('Attempt to generate dynamic folder query with an unset core dynamic folder query?!');
        }
    ...
    public function savePersonalEmailAccount($userId = '', $userName = '', $forceSave = true)
    {
        ...
        if (isset($opts['serial']) && !empty($opts['serial']))
        {
            ...
            $foldersFound = $this
                ->db
                ->query('SELECT folders.id FROM folders WHERE folders.id LIKE "' . $this
                ->db
                ->quote($this->id) . '"');
            $row = $this
                ->db
                ->fetchByAssoc($foldersFound);

            if (empty($row))
            {
                $this->createFolder($inboxFolders[0] . ' (' . $this->name . ')', "inbound", $focusUser, $this->id);
                ...
                foreach ($inboxFolders as $key => $folder)
                {
                    ...
                    $this->createFolder($folder, "inbound", $focusUser);
                }

                $this->createFolder($mod_strings['LNK_MY_DRAFTS'] . ' (' . $stored_options['sentFolder'] . ')', "draft", $focusUser);
                $this->createFolder($mod_strings['LNK_SENT_EMAIL_LIST'] . ' (' . $stored_options['sentFolder'] . ')', "sent", $focusUser);
                $this->createFolder($mod_strings['LBL_LIST_TITLE_MY_ARCHIVES'], "archived", $focusUser);
            }
            else
            {
                // Update folders
                $foldersFound = $this
                    ->db
                    ->query('SELECT * FROM folders WHERE deleted = 0 AND (folders.id LIKE "' . $this
                    ->db
                    ->quote($this->id) . '" OR ' . 'folders.parent_folder LIKE "' . $this
                    ->db
                    ->quote($this->id) . '")');
                $inboxNames = array_splice($inboxFolders, 1);
                while ($row = $this
                    ->db
                    ->fetchRow($foldersFound))
                {
                    $name = '';
                ...
                foreach ($inboxNames as $newInboxFolder)
                {
                    ...
                    $this->createFolder($newInboxFolder, "inbound", $focusUser);
                }
            }
    ...
    public function getUserNameFromGroupId()
    {
        $r = $this
            ->db
            ->query('SELECT users.user_name FROM users WHERE deleted=0 AND id=\'' . $this->group_id . '\'', true);
        while ($a = $this
            ->db
            ->fetchByAssoc($r))
        {
    ...
    public function findOptimumSettings($useSsl = false, $user = '', $pass = '', $server = '', $port = '', $prot = '', $mailbox = '')
    {
        ...
        $this->getImap()
            ->getErrors(); // clearing error stack
    ...
    public function getGroupsWithSelectOptions($groups = array())
    {
        $r = $this
            ->db
            ->query('SELECT id, user_name FROM users WHERE users.is_group = 1 AND deleted = 0', true);
        if (is_resource($r))
        {
            while ($a = $this
                ->db
        ...
        if ($this->template_id)
        {
            ...
            { // if we haven't sent this user 10 replies in 24hours
                ...
                { // use system default
                    $rName = $this
                        ->db
                        ->query('SELECT value FROM config WHERE name = \'fromname\'', true);
                ...
                else
                {
                    $rAddr = $this
                        ->db
                        ->query('SELECT value FROM config WHERE name = \'fromaddress\'', true);
    ...
    public function handleCreateCase($email, $userId)
    {
        ...
        if (!$this->handleCaseAssignment($email) && $this->isMailBoxTypeCreateCase())
        {
            ...
            if (!empty($createCaseTemplateId))
            {
                ...
                $email = $email
                    ->et
                    ->handleReplyType($email, "reply");
                $ret = $email
                    ->et
                    ->displayComposeEmail($email);
    ...
    public function getMessageTextFromSingleMimePart($msgNo, $section, $structure)
    {
        ...
        {
            LoggerManager::getLogger()
                ->fatal('Inbound Email Connection in not a valid resource for getting message text from a single mime part.');

            return false;
        }
        $msgPartTmp = $this->getImap()
            ->fetchBody($msgNo, $section);
     ...
     */
    public function getMessageTextWithUid($uid, $type = null, $structure = null, $fullHeader = null, $clean_email = true, $bcOffset = '')
    {
        $emailBody = $this
            ->imap
            ->fetchBody($uid, '', FT_UID);

        if (!empty($type) && strtolower($type) === 'text/plain')
        {
            $emailMessage = $this
                ->mailParser
                ->parse($emailBody)->getTextContent();
        ...
        $emailMessage = $this
            ->mailParser
            ->parse($emailBody)->getHtmlContent();
        $emailMessage = $this->handleInlineImages($emailBody, $emailMessage);
        $emailMessage = $this->customGetMessageText($emailMessage);
    ...
    protected function handleInlineImages($email, $emailHTML)
    {
        foreach ($this
            ->mailParser
            ->parse($email)->getAllAttachmentParts() as $attachment)
        {
            ...
            if ('inline' === $disposition)
            {
                ...
                foreach ($this->tempAttachment as $temp)
                {
                    if ($temp === $fileName)
                    {
                        ...
                        $fileType = false === $pos ? $fileName : substr($fileName, $pos + 1);
        ...
        foreach ($parts as $k => $part)
        {
            ...
            elseif ((strtolower($part->subtype) == strtolower($subtype)) || (isset($part->disposition) && strtolower($part->disposition) == 'inline' && in_array(strtoupper($part->subtype) , $this->imageTypes)))
            {
                ...
                $stackedBreadcrumbs[] = array(
                    strtolower($part->subtype) => $thisBc
                );
            }
            elseif (5 == $part->type)
            {
    ...
    {
        $imapDecode = $this->getImap()
            ->mimeHeaderDecode($name);
        /******************************
        ...
        return ((isset($encoding) && strtolower($encoding) == 'utf-8') ? $name : isset($encoding)) ? $GLOBALS['locale']->translateCharset($name, $encoding, 'UTF-8') : $name;
    }
    ...
    */
    ...
    public function saveAttachments($msgNo, $parts, $emailId, $breadcrumb, $forDisplay = null)
    {
        ...
        */
        ...
        foreach ($parts as $k => $part)
        {
            ...
            elseif (2 == $part->type && isset($part->subtype) && (strtolower($part->subtype) == 'rfc822' || strtolower($part->subtype) == 'delivery-status'))
            {
                $attach = $this->getNoteBeanForAttachment($emailId);
    ...
    public function saveAttachmentBinaries($attach, $msgNo, $thisBc, $part, $forDisplay)
    {
        ...
        if (!file_exists($uploadDir . $fileName))
        {
            ...
            {
                LoggerManager::getLogger()
                    ->fatal('Inbounc Email Connection is not valid resource for saving attachment binaries.');

                return false;
            }
            $msgPartRaw = $this->getImap()
                ->fetchBody($msgNo, $thisBc);
        ...
        if ((strtolower($part->disposition) == 'inline' && in_array($part->subtype, $this->imageTypes)) || (5 == $part->type))
        {
    ...
    {
        $subjectDecoded = $this->getImap()
            ->MimeHeaderDecode($subject);
    ...
    public function getDuplicateEmailId($msgNo, $uid)
    {
        ...
        {
            LoggerManager::getLogger()
                ->fatal('Inbounc Email Connection is not valid resource for getting duplicate email id.');

            return false;
        }
        $header = $this->getImap()
            ->getHeaderInfo($msgNo);
        $fullHeader = $this->getImap()
            ->fetchHeader($msgNo); // raw headers
    ...
    public function importOneEmail($msgNo, $uid, $forDisplay = false, $clean_email = true)
    {
        ...
        {
            LoggerManager::getLogger()
                ->warn('Connection is not a valid resource for importOneEmail()');
            $header = null;
            $fullHeader = null;
        }
        else
        ...
        if (empty($header))
        {
            ...
            {
                $this
                    ->email->name = $app_strings['LBL_EMAIL_ERROR_MESSAGE_DELETED'];
                $q = "DELETE FROM email_cache WHERE message_id = '{$queryUID}' AND ie_id = '{$this->id}' AND mbox = '{$this->mailbox}'";
            }
            else
            {
                $this
                    ->email->name = $app_strings['LBL_EMAIL_ERROR_IMAP_MESSAGE_DELETED'];
            ...
            $this
                ->email->date_sent_received = $timedate->nowDb();
        ...
        if ($forDisplay || $dupeCheckResult)
        {
            ...
            $structure = $this->getImap()
                ->fetchStructure($msgNo); // map of email
            ///////////////////////////////////////////////////////////////////
    ...
    public function returnImportedEmail($msgNo, $uid, $forDisplay = false, $clean_email = true, $isGroupFolderExists = false)
    {
        ...
        {
            $msgNo = $this->getImap()
                ->getMessageNo((int)$uid);
        }

        $fullHeader = $this->getImap()
            ->fetchHeader($msgNo);
        $header = $this->getImap()
            ->rfc822ParseHeaders($fullHeader);
        $message_id = isset($header->message_id) ? $header->message_id : '';
        ...
        if ($forDisplay || $dupeCheckResult)
        {
            ...
            $contentType = $this
                ->mailParser
                ->parse($emailBody)->getHeaderValue('Content-Type');

            if (!empty($contentType) && strtolower($contentType) === 'text/plain')
            {
                $email->description = $this->getMessageTextWithUid($uid, $contentType, $structure = null, $fullHeader = null, true);
            }

            $email->description_html = $this->getMessageTextWithUid($uid, $structure->subtype, $structure, $fullHeader, $clean_email);
        ...
        else
        {
            ...
            if (!empty($this->compoundMessageId))
            {
                ...
                $result = $this
                    ->db
                    ->query('SELECT id from emails WHERE message_id ="' . $this->compoundMessageId . '"' . 'AND mailbox_id = "' . $this->id . '"');
                $row = $this
                    ->db
                    ->fetchRow($result);
                if (!empty($row['id']))
    ...
    public function returnNonImportedEmail($request)
    {
        ...
        if (empty($header))
        {
            ...
            if (isset($request['metadata']['viewdefs']))
            {
                ...
                {
                    $possibleFormats = [DateTime::RFC2822 . '+', str_replace(['D, '], '', DateTime::RFC2822) ,
                    ...
                    DateTime::RFC822, str_replace(['D, '], '', DateTime::RFC822) ,
                    // day is optional
                    ...
                    if (false === $dateTime)
                    {
                        throw new Exception(sprintf('Expected header Date to comply with RFC882 or RFC2882, but actual is "%s"', $parsedFullHeader->date));
                    }
                ...
                if (in_array('cc_addrs', $fields_selected) || in_array('cc_addrs_names', $fields_selected))
                {
                ...
                $email->description_html = $this->getMessageTextWithUid($uid, $contentType, $structure = null, $fullHeader = null, true);
            }
            else
            {
    ...
    public function setAutoreplyStatus($addr)
    {
        ...
        $this
            ->db
            ->query('INSERT INTO inbound_email_autoreply (id, deleted, date_entered, date_modified, autoreplied_to, ie_id) VALUES (
                            \'' . create_guid() . '\',
                            0,
                            \'' . $timedate->nowDb() . '\',
                            \'' . $timedate->nowDb() . '\',
    ...
    public function getAutoreplyStatus($from)
    {
        ...
        $q_clean = 'UPDATE inbound_email_autoreply SET deleted = 1 WHERE date_entered < \'' . $timedate->getNow()
            ->modify("-24 hours")
            ->asDb() . '\'';
        $r_clean = $this
            ->db
    ...
    public function getCaseIdFromCaseNumber($emailName, $aCase)
    {
        ...
        if ($sub = stristr($emailName, $open))
        {
            ...
            if (ctype_digit($sub3))
            {
                ...
                $query = 'SELECT id FROM cases WHERE case_number = ' . $this
                    ->db
                    ->quoted($sub3) . ' and deleted = 0';
                $results = $this
                    ->db
                    ->query($query, true);
                $row = $this
    ...
    public function getRelatedId($email, $module)
    {
        ...
        if (strpos($email, ',') !== false)
        {
            ...
            foreach ($emailsArray as $emailAddress)
            {
                ...
                $emailAddressString .= $this
                    ->db
                    ->quoted(trim($emailAddress));
            } // foreach
    ...
    public function getNewMessageIds()
    {
        ...
        if (!is_bool($storedOptions) && $storedOptions['only_since'])
        { // POP3 does not support Unseen flags
            ...
            $ret = $this->getImap()
                ->search('SINCE "' . $date . '" UNDELETED UNSEEN');
            $check = $this->getImap()
                ->check();
        ...
        else
        {
            ...
            {
                LoggerManager::getLogger()
                    ->fatal('Inbound Email Connection is not valid resource for getting New Message Ids.');

                return false;
            }
            $ret = $this->getImap()
                ->search('UNDELETED UNSEEN');
        }

        LoggerManager::getLogger()
            ->debug('-----> getNewMessageIds() got ' . count($ret) . ' new Messages');
    ...
    public function connectMailserver($test = false, $force = false)
    {
        ...
        $useSsl = ('true' == $requestSsl) ? true : false; // TODO: validate the ssl request variable value (for e.g its posibble to give a numeric 1 as true)
        if ($test)
        {
            $this->getImap()
                ->setTimeout(1, 15); // 60 secs is the default
            $this->getImap()
                ->setTimeout(2, 15);
            $this->getImap()
                ->setTimeout(3, 15);
        ...
        */
            ...
            ->getConnection()))
        {
                ...
                ->ping())
            {
                ...
                $this->getImap()
                    ->reopen($connectString, CL_EXPUNGE);
            }
        }
        ...
        if ($test)
        {
            if (false === $opts && !is_resource($this->getImap()
                ->getConnection()))
            {
                $this->conn = $this->getImapConnection($connectString, $this->email_user, $this->email_password, CL_EXPUNGE);
            }
            ...
            $this->getImap()
                ->getErrors(); // collapse error stack
            if (is_resource($this->getImap()
                ->getConnection()))
            {
                $this->getImap()
            ...
            {
                LoggerManager::getLogger()
                    ->warn('Connection is not a valid resource.');
            }

            return $msg;
        }
        elseif (!is_resource($this->getImap()
            ->getConnection()))
        {
    ...
    protected function getImapConnection($mailbox, $username, $password, $options = 0)
    {
        ...
        while (!$connection && ($authenticator = array_shift($authenticators)) !== null)
        {
            if ($authenticator)
            {
                $params = ['DISABLE_AUTHENTICATOR' => $authenticator, ];
            }
            else
            {
                $params = [];
            ...
            $connection = $this->getImap()
                ->open($mailbox, $username, $password, $options, 0, $params);
        }
    ...
    public function get_list_view_data()
    {
        ...
        if (!isset($app_list_strings['dom_mailbox_type'][$this
            ->mailbox_type]))
        {
            LoggerManager::getLogger()
                ->fatal('Language string not found for app_list_string[dom_mailbox_type][' . $this->mailbox_type . ']');
        }
        $temp_array['MAILBOX_TYPE_NAME'] = isset($app_list_strings['dom_mailbox_type'][$this->mailbox_type]) ? $app_list_strings['dom_mailbox_type'][$this->mailbox_type] : null;
        //cma, fix bug 21670.
    ...
    public function moveEmails($fromIe, $fromFolder, $toIe, $toFolder, $uids, $copy = false)
    {
        ...
        if ($fromIe == $toIe)
        {
            ...
            {
                if ($this->getImap()
                    ->mailCopy($uids, $toFolder, CP_UID))
                {
                    $this->mailbox = $toFolder;
                    $this->connectMailserver();
                    $newOverviews = $this->getImap()
                        ->fetchOverview($uids, FT_UID);
        ...
        elseif ('folder' == $toIe)
        {
            ...
            if (!isset($_REQUEST['delete']))
            {
                ...
                $ret = $this
                    ->email
                    ->et
                    ->getImportForm($_REQUEST, $this->email);
                $ret['move'] = true;
    ...
    public function deleteMessageOnMailServerForPop3($uid)
    {
        ...
        {
            LoggerManager::getLogger()
                ->fatal('Inbound Email connection is not a resource for deleting Message On Mail Server For Pop3');
    ...
    public function setEmailForDisplay($uid, $isMsgNo = false, $setRead = false, $forceRefresh = false)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $msgNo = ($isMsgNo) ? $uid : $this->getImap()
                    ->getMessageNo($uid);
            }
            if (empty($this->conn))
            {
                ...
                if ("false" == $status)
                {
                    $this->email = BeanFactory::newBean('Emails');
                    $this
                        ->email->name = $app_strings['LBL_EMAIL_ERROR_MAILSERVERCONNECTION'];
                    $ret = 'error';
    ...
    public function setStatuses($uid, $field, $value)
    {
        ...
        $overviews = $this->getCacheValueForUIDs($this->mailbox, array(
            $uid
        ));
...
EOQ;
                $this
                    ->email
                    ->cid2Link($a['id'], $a['file_mime_type']);
            } // while
...
eoq;
        ...
        // toasddrs
        $meta['email']['toaddrs'] = $this->collapseLongMailingList($this
            ->email
            ->to_addrs);
        $meta['email']['cc_addrs'] = $ccs;
        ...
        $return = array();
        $meta['email']['name'] = to_html($this
            ->email
            ->name);
        $meta['email']['from_addr'] = (!empty($this
            ->email
            ->from_addr_name)) ? to_html($this
            ->email
            ->from_addr_name) : to_html($this
            ->email
            ->from_addr);
        isValidEmailAddress($meta['email']['from_addr']);
        $meta['email']['toaddrs'] = (!empty($this
            ->email
            ->to_addrs_names)) ? to_html($this
            ->email
            ->to_addrs_names) : to_html($this
            ->email
            ->to_addrs);
        $meta['email']['cc_addrs'] = to_html($this
            ->email
            ->cc_addrs_names);
        $meta['email']['reply_to_addr'] = to_html($this
            ->email
            ->reply_to_addr);
        $return['meta'] = $meta;
    ...
    public function sortFetchedOverview($arr, $sort = 4, $direction = 'DESC', $forceSeen = false)
    {
        ...
        {
            $arr[$k2]->date = $timedate->fromString($arr[$k2]->date)
                ->asDb();
            $retArr[] = $arr[$k2];
        }
    ...
    public function displayFolderContents($mbox, $forceRefresh = 'false', $page = 1)
    {
        ...
        if (!empty($_REQUEST['sort']) && !empty($_REQUEST['dir']))
        {
            $this
                ->email
                ->et
                ->saveListViewSortOrder($_REQUEST['ieId'], $_REQUEST['mbox'], $_REQUEST['sort'], $_REQUEST['dir']);
            $sort = $_REQUEST['sort'];
    ...
    public function displayFetchedSortedListXML($ret, $mbox)
    {
        ...
        foreach ($ret['retArr'] as $msg)
        {
            ...
            $date = $timedate->to_display_date_time($this
                ->db
                ->fromConvert($msg->date, 'datetime'));
    ...
    {
        $emailUserQuoted = $this
            ->db
            ->quote($this->email_user);
    ...
    public function getNewEmailsForSyncedMailbox()
    {
        ...
        {
            LoggerManager::getLogger()
                ->fatal('Inbound Email connection is not a resource for getting New Emails For Synced Mailbox');
            ...
            if (count($msgIds) > 0)
            {
                ...
                */
                ...
                foreach ($msgIds as $k => & $msgNo)
                {
                    ...
                    array_push($tmpMsgs, array(
                        'msgNo' => $msgNo,
                        'msgId' => $this->compoundMessageId,
                        'exists' => 0
                    ));
                    if ($counter == $limit)
                    {
