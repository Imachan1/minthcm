   ...
   public function handleSave($prefix, $redirect = true, $useRequired = false) {
      ...
      if ( (isset($_POST['isSaveFromDetailView']) && $_POST['isSaveFromDetailView'] == 'true') ||
         ...
         if ( $focus->status == 'Held' && $this->isEmptyReturnModuleAndAction() && !$this->isSaveFromDCMenu() ) {
            ...
            $q = 'SELECT mu.candidate_id, mu.accept_status FROM calls_candidates mu WHERE mu.call_id = \'' . $focus->id . '\' AND mu.deleted=0';
            $r = $focus->db->query($q);
            ...
            $q = 'SELECT mu.resource_id FROM calls_resources mu WHERE mu.call_id = \'' . $focus->id . '\' AND mu.deleted=0';
            $r = $focus->db->query($q);
