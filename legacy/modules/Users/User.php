...
class User extends Person implements EmailInterface
{
    ...
    public function getSignatureButtons($jscall = '', $defaultDisplay = false)
    {
        ...
        $butts = "<input class='button' onclick='javascript:{$jscall}(\"\", \"{$this->id}\");' " .
            "value='{$mod_strings['LBL_BUTTON_CREATE']}' type='button'>&nbsp;";
        if ($defaultDisplay) {
            $butts .= '<span name="edit_sig" id="edit_sig" style="visibility:inherit;"><input ' .
                'class="button" onclick="javascript:' . $jscall .
                '(document.getElementById(\'signature_id\', \'\').value)" ' .
                'value="' . $mod_strings['LBL_BUTTON_EDIT'] . '" type="button" tabindex="392">&nbsp;
					</span>';
        } else {
            $butts .= '<span name="edit_sig" id="edit_sig" style="visibility:hidden;"><input class="button" ' .
                'onclick="javascript:' . $jscall . '(document.getElementById(\'signature_id\', \'\').value)" ' .
                'value="' . $mod_strings['LBL_BUTTON_EDIT'] . '" type="button" tabindex="392">&nbsp;
					</span>';
        }
    ...
    {
        require_once 'include/Exceptions/RequestErrorException.php';
        if (!isset($_REQUEST['record']) || !$_REQUEST['record']) {
            throw new RequestErrorException('Error: requested record is not set');
        }
        $user = BeanFactory::getBean('Users', $_REQUEST['record']);
        if (!$user) {
            throw new RequestErrorException('Error: retrieve requested user record');
        }
        $uid = $user->id;
        if (!$uid) {
            throw new RequestErrorException('Error: retrieve requested user ID');
        }
    ...
    public function save($check_notify = false)
    {
        ...
        if ($smtp_error || $isUpdate && !is_admin($current_user)) {
            ...
            if (!$tmpUser instanceof User) {
                LoggerManager::getLogger()->fatal('User update error: Temp User is not retrieved at ID ' .
                    $this->id . ', ' . gettype($tmpUser) . ' given');
            }
        ...
        if ($retId !== $this->id) {
            LoggerManager::getLogger()->fatal('save error: User is not saved properly, returned Person ID does ' .
...
EOQ;
    ...
    public function load_user($username_password, $password_encoded = false)
    {
        ...
        if ($_SESSION['loginattempts'] > 5) {
            $GLOBALS['log']->fatal('SECURITY: ' . $this->user_name . ' has attempted to login ' .
                $_SESSION['loginattempts'] . ' times from IP address: ' . $_SERVER['REMOTE_ADDR'] . '.');
        ...
        $row = static::findUserPassword($this->user_name, $username_password);
        if (empty($row) || !empty($GLOBALS['login_error'])) {
            $GLOBALS['log']->fatal('SECURITY: User authentication for ' . $this->user_name .
                ' failed - could not Load User from Database');
    ...
    {
        return static::checkPasswordMD5(md5($password), $user_hash);
    }
    ...
    public static function findUserPassword($name, $password, $where = '', $checkPasswordMD5 = true)
    {
        ...
        if (!empty($result)) {
            ...
            if (!$checkPasswordMD5 || static::checkPasswordMD5($password, $row['user_hash'])) {
                return $row;
            }
        }
        return false;
    ...
    {
        $user_hash = static::getPasswordHash($new_password);
    ...
    public function change_password($username_password, $new_password, $system_generated = '0', $validate_only = false)
    {
        ...
        if (!$current_user->isAdminForModule('Users') || ($current_user->isAdminForModule('Users') && ($current_user->id == $this->id))) {
            ...
            $row = static::findUserPassword($this->user_name, md5($username_password));
            if (empty($row)) {
...
// fn
    ...
    {
        if (isset($this->is_admin) && ('1' == $this->is_admin || 'on' === $this->is_admin)
        ) {
            return true;
        }
    ...
    {
        if (false == $this->user_hash && !$this->is_group && !$this->portal_only && isset($GLOBALS['sugar_config']['passwordsetting']['SystemGeneratedPasswordON']) && $GLOBALS['sugar_config']['passwordsetting']['SystemGeneratedPasswordON']
        ) {
            $backUpPost = $_POST;
            $_POST = array(
                'userId' => $this->id,
    ...
    public function ACLAccess($view, $is_owner = 'not_set', $in_group = 'not_set')
    {
        ...
        global $current_user;
        if('list' === $this->ACLNormalizeViewContext($view) && !$current_user->isAdmin()){
            return false;
        }
        // MintHCM #123323 END
    ...
    public function isGroupPrivate($group_id)
    {
        ...
        if ($group && $group->id && 'private' == $group->group_type) {
            return true;
        }
        return false;
    }
