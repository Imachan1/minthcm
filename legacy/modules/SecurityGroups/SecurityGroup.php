...
LIMIT 1";
   ...
   public static function inherit($focus, $isUpdate) {
      ...
      if (
              (isset($sugar_config['securitysuite_popup_select']) && $sugar_config['securitysuite_popup_select'] == true && isset($_REQUEST['action']) && $_REQUEST['action'] == 'Save') || (!empty($_REQUEST['subpanel_field_name']))
      ) {
            ...
            if ( $defaultGroup['module'] == 'All' || $defaultGroup['module'] == $focus->module_dir ) {
               if ( $focus->module_dir == 'Users' ) {
                  $query = 'insert into securitygroups_users(id,date_modified,deleted,securitygroup_id,user_id,noninheritable) '
                          . "select distinct '" . create_guid() . "'," . $focus->db->convert('', 'today') . ",0,g.id,'$focus->id',1 "
      ...
      if ( !$isUpdate && isset($sugar_config['securitysuite_inherit_creator']) && $sugar_config['securitysuite_inherit_creator'] == true ) {
         ...
         if (array_key_exists($focus->module_dir, $security_modules)) {
            ...
            $query .= ",u.securitygroup_id,'$recordId','$focus->module_dir',"
                    ...
                    . "and (g.group_type = 'standard' or (g.group_type = 'private' and g.assigned_user_id = '$focus->assigned_user_id'))";
   ...
   public static function inherit_parent($focus, $isUpdate) {
      ...
      if ( !$isUpdate && isset($sugar_config['securitysuite_inherit_parent']) && $sugar_config['securitysuite_inherit_parent'] == true
         ...
         foreach ( $focus->field_name_map as $name => $def ) {
            if ( (!isset($def['type']) || ($def['type'] == 'relate' && isset($def['id_name']))) && isset($def['module']) && strtolower($def['module']) != 'users'
            ) {
               if ( isset($_REQUEST[$def['id_name']]) ) {
                  $relate_parent_id = $_REQUEST[$def['id_name']];
                  $relate_parent_type = $def['module'];

                  self::inherit_parentQuery($focus, $relate_parent_type, $relate_parent_id, $focus_id, $focus_module_dir);
    ...
    */
   public static function addGroupToRecord($module, $record_id, $securitygroup_id) 
   {
      ...
      $db = DBManagerFactory::getInstance();
      $query = 'insert into securitygroups_records(id,securitygroup_id,record_id,module,date_modified,deleted) '
              . "values( '" . create_guid() . "','" . $securitygroup_id . "','$record_id','$module'," . $db->convert('', 'today') . ',0) ';
...
order by securitygroups_users.primary_group desc ";
   ...
   public function ACLAccess($view, $is_owner = 'not_set', $in_group = 'not_set') {
      global $action; //MintHcm start #60585
      $result = parent::ACLAccess($view, $is_owner, $in_group);
      if ( $this->id && !$this->group_type ) {
         $this->retrieve($this->id);
      }
      $disabled_views = array( 'edit', 'popupeditview', 'save', 'editview','delete' );
      //MintHcm start #60585
      if($action!=='SubPanelViewer' && $this->group_type == 'private' && in_array(strtolower($view), $disabled_views)){
          $result = false;
      }
      //MintHcm end #60585
      return $result;
