...
class BasePDFGenerator {
   ...
   function getFieldValueForOne($name, $bean, $r) {
      global $sugar_config;
      $field_defs = $bean->field_defs;
   ...
   function prepareTplCode($pdftemplate) {

      $template = str_replace('&nbsp;', ' ', $pdftemplate->template);
   ...
   function returnPdf($filename, $mode = 'I') {
      if ( isset($_GET["file"]) ) {
   ...
   function replaceCountAndCurrency($tpl_str, $relationship, $counter, $currency) {
   ...
   function getRelationshipForParse($relationship) {
      if ( $relationship != '' ) {
   ...
   function isTableOutside($table_outside) {
   ...
   function prepareField(&$field, $bean, $str, $rel) {
   ...
   function prepareFieldValue(&$field, $bean) {
      global $sugar_config;
      if ( $field['type'] == 'currency' ) {
         $curr = new Currency();
         $curr->retrieve($bean->currency_id);
