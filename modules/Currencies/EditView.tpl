        ...
        <td>
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                <tr>
                    ...
                    <td scope="row">
                        <span>{$MOD.CURRENCY_ON_RIGHT}:</span>
                    </td>
                    <td>
                        <span>
                            {if !empty($CURRENCY_ON_RIGHT)}
                                {assign var='currency_on_right_checked' value=' checked'}
                            {else}
                                {assign var='currency_on_right_checked' value=''}
                            {/if}
                            <input type="checkbox" name="currency_on_right" value="1"{$currency_on_right_checked}></span>
                        </span>
                    </td>
                </tr>
                <tr>
                    <td scope="row">
                        <span>{$MOD.LBL_HIDDEN}:</span>
                    </td>
                    <td>
                        <span>
                            {if !empty($HIDDEN)}
                                {assign var='hidden_checked' value=' checked'}
                            {else}
                                {assign var='hidden_checked' value=''}
                            {/if}
                            <input type="checkbox" name="hidden" value="1"{$hidden_checked}>
                        </span>
                    </td>
