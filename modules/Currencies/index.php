...
//$ListView->setHeaderTitle($mod_strings['LBL_LIST_FORM_TITLE']. $header_text );
   ...
   foreach ( $lc->list as $currency_bean ) {
      $currency_bean->currency_on_right = ($currency_bean->currency_on_right) ? $mod_strings['LBL_YES'] : $mod_strings['LBL_NO'];
      $currency_bean->hidden = ($currency_bean->hidden) ? $mod_strings['LBL_YES'] : $mod_strings['LBL_NO'];
   }
...
// Load in the full ISO 4217 list, so we can dynamically populate the currency strings
   ...
   // View Tools start #40674
   $sugar_smarty->assign('CURRENCY_ON_RIGHT', $focus->currency_on_right);
   $sugar_smarty->assign('HIDDEN', $focus->hidden);
   // View Tools end #40674
