...
*/' . "\n");
   ...
   foreach ( $Expressionlist as $key => $filename ) {
      if ( strpos($filename, 'VTExpression_') !== false ) {
         $tmp_expression_list[substr($filename, 13, (strlen($filename) - 17))] = 'include/ViewTools/Expressions/' . $filename;
      }
   }
   ...
   if ( is_dir('include/ViewTools/Expressions/ext/') ) {
      ...
      foreach ( $Expressionlist as $key => $filename ) {
         if ( strpos($filename, 'VTExpression_') !== false ) {
            $tmp_expression_list[substr($filename, 13, (strlen($filename) - 17))] = 'include/ViewTools/Expressions/ext/' . $filename;
         }
      }
   }
   //Write formula definitions to file
   foreach ( $tmp_expression_list as $fname => $src ) {
      fwrite($phpcache, '$vt_expression_list[\'' . $fname . '\'] = \'' . $src . '\';' . "\n");
   }
   fclose($phpcache);
...
window.viewTools.cache.serversideFrontend=' . json_encode($formula_list) . ';');
   ...
   foreach ( $vt_expression_list as $functionName => $fileSource ) {
      ...
      $validationObj = new $validationClass();
      if ( $validationObj->frontend() !== false || $validationObj->serversideFrontend === true ) {
...
' . $returnFormulas);
   ...
   $documentationFileHandler = fopen('include/ViewTools/Expressions/documentation.html', 'w');
   fwrite($documentationFileHandler, file_get_contents('include/ViewTools/Expressions/documentationHeader.html'));
   fwrite($documentationFileHandler, "<b style=\"color:#aaa;\">Please put custom formula classes in ' custom / Expressions / ext / ' directory. </b><br/>\n");
   fwrite($documentationFileHandler, "<i style=\"color:#aaa;\">[Documentation generated : " . date('Y-m-d h:i:s') . "]</i><br/>\n");

   fwrite($documentationFileHandler, "<div style=\"border-top: 1px solid #cccccc;margin-top:5px; padding-top:5px;\">Filter:{$formula_availablilities}</div>\n");

   foreach ( $vt_expression_list as $functionName => $fileSource ) {
      $validationClass = 'VTExpression_' . $functionName;
      $validationObj = new $validationClass();
      fwrite($documentationFileHandler, "<div style=\"border-top: 1px solid #cccccc;margin-top:5px; padding-top:5px;\" class=\"vt_formula " . implode(" ", $validationObj->availability) . "\">\n");
      fwrite($documentationFileHandler, "<div><h1><b><a id=\"{$functionName}\">{$functionName}</a></b>");

      if ( $validationObj->inputParams != false ) {
         fwrite($documentationFileHandler, "<span style=\"color:grey;\"> (" . implode(' , ', $validationObj->inputParams) . ")</span>");
      }
       ...
       */
      $docEndFlag = null;
      while ( ($buffer = fgets($fileHandler, 4096)) !== false && $docEndFlag !== true ) {
         ...
         //Read description
            ...
            //Read description
               ...
               //Predefined tags
               $buffer = preg_replace("/EOU:/", "<i>Example of use:</i>", $buffer);
               //Replace function names as links to formula descriptions
               foreach ( $vt_expression_list as $fname => $null ) {
                  $buffer = preg_replace("/{$fname}\(/", "<a href=\"#{$fname}\">{$fname}</a>(", $buffer);
               }
               //set $field_name as bold
               $buffer = preg_replace('/\$(\w+)/', '<b style="color:black;">\$$1</b>', $buffer);
               //set color to strings
               $buffer = preg_replace('/\'(.+)\'/', '<span style="color:green;">\'$1\'</span>', $buffer);
               //set color to numbers
               $buffer = preg_replace('/\ (\d+)\ /', ' <b style="color:orange;">$1</b> ', $buffer);
               $buffer = preg_replace('/\ -(\d+)\ /', ' <b style="color:orange;">-$1</b> ', $buffer);
               //
      ...
      if ( $validationObj->availability != false ) {
         ...
         fwrite($documentationFileHandler, "<i style=\"color:grey;\">Available in: " . implode(' , ', $validationObj->availability) . " / {$tmpWhereVavailable}</i><br/>\n");
      }
   ...
   foreach ( scandir('cache/modules') as $folder ) {
      if ( $folder != '.' && $folder != '..' && file_exists('cache/modules/' . $folder) && is_dir('cache/modules/' . $folder) ) {
         foreach ( scandir('cache/modules/' . $folder) as $file ) {
            if ( $file !== '.' && $file != '..' && strpos($file, 'vardefs.php') !== false ) {
      ...
      if ( is_array($moduleData['fields']) ) {
         foreach ( $moduleData['fields'] as $fieldName => $params ) {
            foreach ( $params as $paramName => $paramData ) {
               //Found vt formula
               if ( in_array($paramName, array( 'vt_calculated', 'vt_dependency', 'vt_required', 'vt_readonly' )) ) {
                  if ( is_array($paramData) ) {
                     foreach ( $paramData as $paramDataField => $paramDataValue ) {
                        if ( substr($paramDataValue, 0, 1) == '$' ) {
                           $initArray[$moduleData['table']][substr($paramDataValue, 1)][$fieldName] = $fieldName;
                        }
                     }
                     ...
                     foreach ( $variables[1] as $matchedVariable ) {
                        $initArray[$moduleData['table']][$matchedVariable][$fieldName] = $fieldName;
                     }
                     ...
                     foreach ( $variables[1] as $matchedVariable ) {
                        ...
                        if ( $focus->load_relationship($matchedVariable) ) {
                           ...
                           if ( isset($def['join_table']) ) {
                              ...
                              foreach ( $focus->field_defs as $field ) {
                                 if ( $field['id_name'] == $def["join_key_" . $get_foreign_key_from] && $field['link'] == $matchedVariable ) {
                                    $initArray[$moduleData['table']][$field['name']][$fieldName] = $fieldName;
                                    break;
                                 }
                              }
                           } else {
                              ...
                              if ( isset($def['lhs_key']) ) {
                                 $initArray[$moduleData['table']][$def['lhs_key']][$fieldName] = $fieldName;
                              }
                              //rhs_key
                              if ( isset($defs[$matchedVariable]['rhs_key']) ) {
                                 $initArray[$moduleData['table']][$def['rhs_key']][$fieldName] = $fieldName;
                              }
                           }
                           /*
                        ...
                        if ( isset($relationships[$matchedVariable]) ) {
                           ...
                           if ( isset($relationship['lhs_key']) ) {
                              $initArray[$moduleData['table']][$relationship['lhs_key']][$fieldName] = $fieldName;
                           }
                           //rhs_key
                           if ( isset($relationships[$matchedVariable]['rhs_key']) ) {
                              $initArray[$moduleData['table']][$relationship['rhs_key']][$fieldName] = $fieldName;
                           }
...
window.viewTools.cache.formulaRequirements = ' . json_encode($parsed_field_requirements) . ';');
   ...
   foreach ( $GLOBALS["dictionary"] as $module ) {
      if ( isset($module['vt_duplicate']) && is_string($module['vt_duplicate']) ) {
         $variables = array();
         $module['vt_duplicate'] = preg_replace('/\ /', '', $module['vt_duplicate']);
...
window.viewTools.cache.formulaDuplicateFields = ' . json_encode($duplicate_fields) . ';');
   ...
   foreach ( $GLOBALS["dictionary"] as $module ) {
      if ( isset($module['vt_duplicate']) ) {
         $duplicate_formulas .= '$duplicate[\'' . $module['table'] . '\'][\'formula\']=\'' . str_replace("'", "\'", $module['vt_duplicate']) . '\';' . "\n";
         ...
         if ( isset($module['vt_duplicateColumns']) ) {
            $module['vt_duplicateColumns'] = preg_replace('/\ /', '', $module['vt_duplicateColumns']);
            preg_match_all('/(\w+)/i', $module['vt_duplicateColumns'], $columns);
...
' . $duplicate_formulas);
      ...
      if ( is_array($module['fields']) ) {
         foreach ( $module['fields'] as $module_field ) {
            if ( isset($module_field['vt_calculated']) && is_string($module_field['vt_calculated']) ) {
               preg_match_all('/\#(\w+)/i', $module_field['vt_calculated'], $vt_relationsip_tmp);
               if ( isset($vt_relationsip_tmp[1]) && count($vt_relationsip_tmp[1]) > 0 ) {
                  foreach ( $vt_relationsip_tmp[1] as $relationship_name ) {
                     ...
                     if ( $relationship['lhs_table'] == $module['table'] ) {
                        ...
                        foreach ( $focus->field_defs as $field ) {
                           if ( $field['type'] == 'link' && $field['relationship'] == $relationship['name'] ) {
                              $related_tmp[$relationship['lhs_table']][] = "'{$field['name']}'";
                              break;
                           }
                        }
                     }
   ...
   foreach ( $related_tmp as $key => $module ) {
      fwrite($phpcache, "\n\$related_recalculation['{$key}']=array(" . implode(',', array_unique($module)) . ");");
   }
   fclose($phpcache);
