...
class SugarFeedDashlet extends DashletGeneric {
   ...
   public function process($lvsParams = array(), $id = null) {
      ...
      if ( !empty($this->displayTpl) ) {
         ...
         $this->lvs->setup(
                 $this->seedBean, $this->displayTpl, $where, $lvsParams, 0, $this->displayRows, array( 'name',
            'description',
            'date_entered',
            'created_by',
...
enableQS(false);
   ...
   public function display() {
      ...
      $listview = preg_replace_callback(
         ...
         if ( !file_exists($image_path . $matches[1] . '.gif') ) {
            if ( file_exists('custom/themes/default/images/' . $matches[1] . '.gif') ) {
               $image_path = 'custom/themes/default/images/';
            }
         }
         return '<a href="index.php?module=' . $matches[1] . '&action=DetailView&record=' . $matches[2] . '"><img src="' . $image_path . $matches[1] . '.gif" border=0 REPLACE_ALT>' . $matches[3] . '</a>';
      }, $listview); /* SKIP_IMAGE_TAG */
      //View Tools #50754 END
