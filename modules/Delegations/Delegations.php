...
class Delegations extends Delegations_sugar {
   ...
   public function generateDelegationID() {
      ...
      if ( $assigned_user && $assigned_user->id ) {
         ...
         $this->delegation_id = sprintf("%s/%02d/%02d/%d", $assigned_user->user_name, $del_year, $del_month, $seq->getNext($seq_name));
   ...
   public function decrementSequenceValue() {
      ...
      $del_year = $timedate->swap_formats($this->start_date, $timedate->get_date_time_format($current_user), "Y");
      $del_month = $timedate->swap_formats($this->start_date, $timedate->get_date_time_format($current_user), "m");
      $this->start_date = $timedate->swap_formats($this->start_date, $timedate->get_date_time_format($current_user), $timedate::DB_DATETIME_FORMAT);
      $assigned_user = BeanFactory::getBean('Users', $this->assigned_user_id);
      if ( $assigned_user && $assigned_user->id ) {
         $seq = new SequenceUtils();
         $seq_name = $this->generateSequenceName();
         $last_delegation_id = sprintf("%s/%02d/%02d/%d", $assigned_user->user_name, $del_year, $del_month, $seq->getCurrent($seq_name));
   ...
   public function countTransportCost() {
      ...
      $query = "SELECT SUM(transportations.cost_total) as sum FROM transportations 
         WHERE transportations.delegation_id = '" . $this->id . "' 
         AND transportations.deleted=0";
      $result = $this->db->query($query);
      $row = $this->db->fetchByAssoc($result);
      $this->transport_cost = ( $row['sum'] > 0 ) ? $row['sum'] : 0;
   }
   ...
   public function countTotalAccommodation() {
      ...
      $result = $this->db->query($query);
      $row = $this->db->fetchByAssoc($result);
      $this->total_accommodation = ( $row['sum'] > 0 ) ? $row['sum'] : 0;
   }
   ...
   public function countOtherCosts() {
      ...
      $result = $this->db->query($query);
      $row = $this->db->fetchByAssoc($result);
      $this->other = ( $row['sum'] > 0 ) ? $row['sum'] : 0;
   }

   public function countTotalExpenses() {
      $this->total_expenses = $this->other + $this->accommodation_lump_sum + $this->total_accommodation + $this->regiments + $this->transport_cost;
   }

   public function countPayoffSum() {
      $this->payoff_sum = ($this->total_expenses - $this->obtained_sum) > 0 ? ($this->total_expenses - $this->obtained_sum) : 0;
   }

   public function countReturnSum() {
      $this->return_sum = ($this->total_expenses - $this->obtained_sum) < 0 ? ($this->obtained_sum - $this->total_expenses) : 0;
   }
