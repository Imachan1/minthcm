...
class PipelineBySalesStageDashlet extends DashletGenericChart {
   ...
   public $sales_stage_list; //View Tools #42467
   /**
   ...
   $id, array $options = null
      ...
      //View Tools #42467 START
      $oppo = BeanFactory::newBean('Opportunities');
      $this->sales_stage_list = $oppo->field_defs['sales_stage']['options'];
      //View Tools #42467 END
   ...
   public function displayOptions() {
      ...
      $this->_searchFields['pbss_sales_stages']['options'] = $app_list_strings[$this->sales_stage_list]; //View Tools #42467
   ...
   public function display() {
      ...
      // View Tools end #41221
      // View Tools start #36394
      /*
   ...
   $query
      ...
      if ( count($tempx) > 0 ) {
         foreach ( $tempx as $key ) {
            $datax[$key] = $app_list_strings[$this->sales_stage_list][$key]; //View Tools #42467
            $selected_datax[] = $key;
         }
      } else {
         $datax = $app_list_strings[$this->sales_stage_list]; //View Tools #42467
         $selected_datax = array_keys($app_list_strings[$this->sales_stage_list]); //View Tools #42467
      }
      ...
      foreach ( $selected_datax as $sales_stage ) {
         foreach ( $temp_data as $key => $value ) {
            if ( $value['sales_stage'] == $sales_stage ) {
               $value['sales_stage'] = $app_list_strings[$this->sales_stage_list][$value['sales_stage']]; //View Tools #42467
