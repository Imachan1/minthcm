...
class Costs extends Costs_sugar {
   ...
   public function save($check_notify = false) {
      ...
      foreach ( $rel_map as $key => $value ) {
         if ( $this->$value && ( $this->cost_amount != $this->fetched_row[$value] || $this->$value != $this->fetched_row[$value] ) ) {
   ...
   public function setCostFields() {
      ...
      $this->name = sprintf("%s %s %s", translate('LBL_TITLE', 'Costs'), $this->cost_date, translate('cost_type', '', $this->type));
   }
   ...
   public function convertCostAmount() {
      ...
      if ( $this->currency_id != '-99' ) {
         if ( $this->delegation_id ) {
            $this->cost_amount_usdollars = $this->cost_amount * $this->getExchangeRateFromDelegation($this->delegation_id);
         } else {
   ...
   protected function getExchangeRateFromDelegation($delegation_id) {
