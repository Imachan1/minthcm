...
// fn
   ...
   private function mesageStructureHasAttachment($imapStructure) {
      if ( !isset($imapStructure->parts) && isset($imapStructure->disposition) && $imapStructure->disposition == 'attachment' ) {
         return true;
      }
   ...
   public function findOptimumSettings(
      ...
      foreach ( $serviceArr as $k => $serviceTest ) {
         ...
         if ( ($errors = $this->getImap()->getLastError()) || ($alerts = $this->getImap()->getAlerts()) ) {
            ...
            if ( $errors == 'Too many login failures' || $errors == '[CLOSED] IMAP connection broken (server response)'
   ...
   public function returnNonImportedEmail($request) {
      ...
      if ( empty($header) ) {
         ...
         if ( isset($request['metadata']['viewdefs']) ) {
            ...
            if (
                    in_array('cc_addrs', $fields_selected) || in_array('cc_addrs_names', $fields_selected)
            ) {
...
// fn
   ...
   public function insertMailBoxFolders($value) {
      ...
      if ( empty($a['value']) ) {
         ...
         $id = create_guid();
         $query = "INSERT INTO config VALUES('{$id}', 'InboundEmail', '{$this->id}', $value)";
         $this->db->query($query);
      } // if
   }
