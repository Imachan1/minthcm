...
class Importer {
         ...
         case 'multienum':
            ...
            if ( $returnValue === false ) {
               $this->importSource->writeError(
                       $mod_strings['LBL_ERROR_NOT_IN_ENUM'] . implode(",", $app_list_strings[$fieldDef['options']]), $fieldTranslated, $rowValue
               );
               return false;
            }
            return $returnValue;
   ...
   protected function saveImportBean($focus, $newRecord) {
      ...
      // View Tools start #33573
      $focus->skip_vt_validation = true;
      // View Tools end #33573
      $focus->save(false);
   ...
   protected function saveMappingFile() {
      ...
      $result = $mapping_file->save(
              $current_user->id, $_REQUEST['save_map_as'], $_REQUEST['import_module'], $_REQUEST['source'], (isset($_REQUEST['has_header']) && $_REQUEST['has_header'] == 'on'), $_REQUEST['custom_delimiter'], html_entity_decode($_REQUEST['custom_enclosure'], ENT_QUOTES)
      );
   }
