...
/* View Tools
 * #55927: QuickSearch dla relacji z modułem Employees 
 */
/**
...
class quicksearchQuery {
   ...
   protected function constructWhere($focus, $args) {
            ...
            case self::CONDITION_CONTAINS:
               array_push(
                  $conditionArray, sprintf(
                     "%s like '%%%s%%'", $table_prefix . $db->getValidDBName($condition['name']), $db->quote(
                        $condition['value']
                     )
                  )
               );
      ...
      //View Tools start #55927
      //if ($table == 'users') {
      if ( get_class($focus) == 'User' ) {
         //View Tools end #55927
   ...
   protected function updateContactName($result, $args) {
      ...
      $result[$args['field_list'][0]] = $locale->getLocaleFormattedName(
         $result['first_name'], $result['last_name'], $result['salutation']
      );
   ...
   protected function overrideContactId($result, $data, $args) {
      foreach ( $args['field_list'] as $field ) {
         $result[$field] = (preg_match('/reports_to_id$/s', $field) || preg_match('/contact_id$/s', $field)) ? $data->id // "reports_to_id" to "id"
            : $data->$field;
      }
   ...
   protected function getUserArray($condition) {
      return (showFullName())
         ...
         ? getUserArrayFromFullName($condition, true) : get_user_array(false, 'Active', '', false, $condition, ' AND portal_only=0 ', false);
   }
         ...
         foreach ( $args['conditions'] as $i => $condition ) {
            if ( isset($condition['name'], $condition['value']) ) {
               switch ( $condition['name'] ) {
                  case 'name':
                     $where[] = sprintf(
                        "(teams.name like '%s%%' OR teams.name_2 like '%s%%')", $db->quote($condition['value']), $db->quote($condition['value'])
                     );
                     unset($args['conditions'][$i]);
                     break;
                  case 'user_id':
                     $where[] = sprintf(
                        "teams.id IN (SELECT team_id FROM team_memberships WHERE user_id = '%s' AND deleted = 0)", $db->quote($condition['value'])
                     );
                     unset($args['conditions'][$i]);
                     $teams_filtered = true;
               }
