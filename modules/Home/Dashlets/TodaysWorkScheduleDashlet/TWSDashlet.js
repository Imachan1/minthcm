if (!window.TWSDashlet) {
   TWSDashlet = function (innerEl) {
      ...
      this.$evCloseButton = this.$toolForm.find('#evCloseButton');
      ...
      this.$planSelect.change(function () {
         ...
         if (!this.value) {
            ...
            _this.$evCloseButton.parent().hide();
         } else {
            _this.$content.show();
            ...
            _this.$evCloseButton.parent()[isClosed ? 'hide' : 'show']();
   ...
   TWSDashlet.init = function () {
      TWSDashlet.initRedmineProjectTasksDashlet();
   ...
   TWSDashlet.initRedmineProjectTasksDashletId = function (dashletId) {
      var $dashletEntire = $('#dashlet_entire_' + dashletId);
      $dashletEntire.find("table b").draggable({
         cursor: "move",
         opacity: 0.7,
         helper: "clone",
   ...
   TWSDashlet.initRedmineProjectTasksDashlet = function () {
      if (Array.isArray(window.redmine_projekttasks_dashlet_id)) {
         for (var i = 0; i < window.redmine_projekttasks_dashlet_id.length; i++) {
            TWSDashlet.initRedmineProjectTasksDashletId(window.redmine_projekttasks_dashlet_id[i]);
         }
      } else {
         if (typeof window.redmine_projekttasks_dashlet_id == "string") {
            TWSDashlet.initRedmineProjectTasksDashletId(window.redmine_projekttasks_dashlet_id);
         } else {
            console.warn('Redmine Projekttask Dashlet not found');
         }
      }
   };
   TWSDashlet.initRedmineProjectTasksDashletOld = function () {
      if (window.redmine_projekttasks_dashlet_id) {
         var dashletId = window.redmine_projekttasks_dashlet_id,
                 $dashletEntire = $('#dashlet_entire_' + dashletId);
         $dashletEntire.find("table b").draggable({
            cursor: "move",
            opacity: 0.7,
            helper: "clone",
