...
> ⚠️ **CRITICAL — commit sequence is strictly ordered:**
> 1. Display summary → 2. Ask for reviewer → 3. Wait for user response → 4. Confirm login → 5. **Only then run `git commit`**
>
> **Never run `git commit` before the reviewer name is received and confirmed.**
> **Never use conventional commit prefixes** (`feat:`, `fix:`, `chore:` etc.) — the only valid format is `ref #{ISSUE_ID} ... as mentioned below`.

### Commit flow

1. Display a summary **and stop — wait for the user to provide the reviewer**:
...
2. **Wait for the user's reply. Do not commit yet.**

   Once the user provides a name, normalize it:
...
3. **After confirmation**, commit with the reviewer:
   ```bash
   git add -A
...
4. **Do not push** without explicit request from the user.

### Exception: early commit

If the user explicitly asks to commit mid-implementation, comply — but use the **same format
and reviewer flow** (ask for reviewer, confirm, then commit). Note in `plan.md` that a
