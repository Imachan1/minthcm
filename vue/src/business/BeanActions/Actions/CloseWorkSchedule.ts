...
export class CloseWorkSchedule extends BeanAction {
    ...
    public static readonly DONT_CHECK_TYPES = ['holiday', 'sick', 'occasional_leave', 'overtime', 'excused_absence', 'leave_at_request', 'child_care']
    ...
    public isAvailable(): boolean {
        return super.isAvailable() && ['WorkSchedules'].includes(this.bean.module) && !['closed'].includes(this.bean.attributes.status)
    }
    ...
    public async execute() {
        ...
        if(
            CloseWorkSchedule.DONT_CHECK_TYPES.includes(this.bean.attributes.type)
            || (await this.canBeClosed())
        ){
    ...
    async canBeClosed()
    {
        ...
        if(response?.data?.result != '1'){
            await usePopupsStore().alert(useLanguagesStore().label(CloseWorkSchedule.VALIDATION_MESSAGES[response?.data?.result], 'WorkSchedules', {"name": this.bean.name}))
            return false
        }
        return true;
    }
