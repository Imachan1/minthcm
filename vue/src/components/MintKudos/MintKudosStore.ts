...
export const useMintKudosStore = defineStore('mint-kudos', () => {
    ...
    async function fetchInitialData() {
        ...
        const response = await mintApi.get<InitialResponse>(`api/kudos/init`)
    ...
    async function addKudos(gifted_user_id: string, message: string, isPrivate: boolean, id: string) {
        try {
            isLoading.value = true
            const response = await mintApi.post(`api/kudos/add`, {
                id,
                gifted_user_id,
                message: message,
                private: isPrivate,
    ...
    async function deleteKudos(id: string) {
        try {
            ...
            const response = await mintApi.delete(`api/kudos/${id}`)
            if (response.status === 200) {
    ...
    async function reactToKudos(id: string, reactionType: string) {
        ...
        await mintApi.post(`api/reactions/Kudos/${id}`, {
    ...
    async function deleteKudosReaction(id: string) {
        ...
        await mintApi.delete(`api/reactions/Kudos/${id}`)
    }
