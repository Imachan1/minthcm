...
class SugarFieldRelate extends SugarFieldBase {
   ...
   public function getEditViewSmarty($parentFieldArray, $vardef, $displayParams, $tabindex) {
      ...
      $call_back_function = 'viewTools.form.function.set_return'; //View Tools
      ...
      // MintHCM START
      // #74195 - zakomentowanie ponizszej linii poniewaz dodawala zbedne slashe w urlce do popupu na zadanym initial_filter
   ...
   $value, $vardef, $focus, ImportFieldSanitize $settings
      ...
      if ( $vardef['module'] == 'Users' && isset($vardef['rname']) && $vardef['rname'] == 'user_name' ) {
         $userFocus = new User;
         $query = sprintf(
                 "SELECT user_name FROM {$userFocus->table_name} WHERE %s=%s AND deleted=0", $userFocus->db->concat('users', array( 'first_name', 'last_name' )), $userFocus->db->quoted($value)
         );
