...
require_once 'custom/include/LastNextContacts/LastNextContactsQueue.php';
require_once 'custom/include/LastNextContacts/LastNextContactsDeadCode.php';

require_once 'custom/include/LastNextContacts/LastNextContactsQueries.php';
require_once 'custom/include/LastNextContacts/LastNextContactsEmail.php';
...
class LastNextContacts extends LastNextContactsDeadCode
{
    ...
    public function updateLastNextContactDate($bean, $related_beans)
    {
        ...
        && 'Emails' != $bean->module_name
            ...
            if (isset($bean->accounts)) {
                $beans = $bean->accounts->getBeans();
                if (count($beans) > 0) {
                    $this->getAccounts($beans);
                }
            }
    ...
    public function updateBean($arguments)
    {
        ...
        && in_array($arguments['related_module'], LastNextContactsConfig::get('last_next_modules'))
            ...
            if ('Contacts' == $bean_to_update->module_name && strlen($bean_to_update->account_id) > 0) {
                $this->updateAccount($bean_to_update->account_id);
            }
        }
    }

    protected function updateAccount($id)
    {
        $account_bean = $this->getBean('Accounts', $id);
        ...
        if (!empty($ids) && is_array($ids)) {
            foreach ($ids as $id) {
                ...
                if (strlen($bean->account_id) > 0) {
                    $account_bean = $this->getBean('Accounts', $bean->account_id);
    ...
    protected function findContactDateActivity($bean, $activities, $activity, $func)
    {
        ...
        switch ($bean->module_name) {
            // fabryka
            case 'Accounts':
                $sql = $this->getLNCQueries()->Accounts($status, $bean->id, $activity, $activities, $sort);
                break;
            case 'Leads':
                $sql = $this->getLNCQueries()->Leads($status, $bean->id, $activity, $activities, $sort);

                break;
            case 'Prospects':
                $sql = $this->getLNCQueries()->Prospects($status, $bean->id, $activity, $activities, $sort);
                break;
            case 'Contacts':
                $sql = $this->getLNCQueries()->Contacts($status, $bean->id, $activity, $activities, $sort);
                break;
        }
