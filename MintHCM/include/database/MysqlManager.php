...
class MysqlManager extends DBManager {
   ...
   public function connect(array $configOptions = null, $dieOnError = false) {
      ...
      if ( $this->getOption('persistent') ) {
         $this->database = @mysql_pconnect(
                         $configOptions['db_host_name'], $configOptions['db_user_name'], $configOptions['db_password']
         );
      }
         ...
         if ( empty($this->database) ) {
            ...
            if ( $dieOnError ) {
               if ( isset($GLOBALS['app_strings']['ERR_NO_DB']) ) {
                  ...
                  sugar_die("Could not connect to the database. Please refer to minthcm.log for details.");
               }
            } else {
               return false;
            }
   ...
   public function massageFieldDef(&$fieldDef, $tablename) {
      ...
      if ( isset($fieldDef['default']) &&
              ($fieldDef['dbType'] == 'text' || $fieldDef['dbType'] == 'blob' || $fieldDef['dbType'] == 'longtext' || $fieldDef['dbType'] == 'longblob' ) ) {
