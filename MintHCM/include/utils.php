...
function make_sugar_config(&$sugar_config)
{
    ...
    $sugar_config = array(
        ...
        'resource_management' => array(
            ...
            'special_query_modules' => array('Reports', 'Export', 'Import', 'Administration',
                'Sync'),
            'default_limit' => 1000,
        ),
...
function return_mod_list_strings_language($language, $module)
{
    ...
    if ($currentModule == $module && isset($mod_list_strings) && $mod_list_strings
        != null) {
...
function get_workflow_admin_modules_for_user($user)
{
    ...
    if (isset($actions['ForecastSchedule']['module']['admin']['aclaccess']) && ($actions['ForecastSchedule']['module']['admin']['aclaccess']
        == ACL_ALLOW_DEV ||
    ...
    foreach ($workflow_mod_list as $key => $val) {
        if (!in_array($val, $workflow_admin_modules) && ($val != 'iFrames' && $val
            != 'Feeds' && $val != 'Home' && $val != 'Dashboard' && $val != 'Calendar'
            && $val != 'Activities' && $val != 'Reports') &&
                                                 ...
                                                 $massupdate = false)
{
    ...
    foreach ($key_list as $option_key => $option_value) {
        ...
        if (
            ...
            $option_key == '' && (($selected_key == '' && !$massupdate) || $selected_key
...
function translate($string, $mod = '', $selectedValue = '')
{
   ...
   //    echo("Total time: $total_time<br>");
    ...
    if (is_array($returnValue) && (!empty($selectedValue) || (is_numeric($selectedValue)
        && $selectedValue == 0)) && isset($returnValue[$selectedValue])) {
...
function clean_incoming_data()
{
    ...
    if (isset($_REQUEST['lvso'])) {
        set_superglobals('lvso',
            (strtolower($_REQUEST['lvso']) === 'desc') ? 'desc' : 'asc');
    }
...
function securexss($value)
{
    ...
    static $xss_cleanup = array('&quot;' => '&#38;', '"' => '&quot;', "'" => '&#039;',
        '<' => '&lt;', '>' => '&gt;');
    $value              = preg_replace(array('/javascript:/i', '/\0/'),
        array('java script:', ''), $value);
    $value              = preg_replace('/javascript:/i', 'java script:', $value);

    return str_replace(array_keys($xss_cleanup), array_values($xss_cleanup),
        $value);
}
...
function clone_history(&$db, $from_id, $to_id, $to_type)
{
    ...
    foreach ($tables as $table => $bean_class) {
        ...
        while ($row     = $db->fetchByAssoc($results)) {
            ...
            if ($to_type == 'Contacts' and in_array('contact_id',
                    $bean->column_fields)) {
...
)
{
    ...
    if (!$user_array) {
        ...
        /* BEGIN - SECURITY GROUPS */
        global $current_user, $sugar_config;
        if ($focus->module_dir == 'Users' && !is_admin($current_user) && isset($sugar_config['securitysuite_filter_user_list'])
            && $sugar_config['securitysuite_filter_user_list'] == true
        ) {
            require_once 'modules/SecurityGroups/SecurityGroup.php';
            $group_where = SecurityGroup::getGroupUsersWhere($current_user->id);
            $query       .= ' AND ('.$group_where.') ';
        } elseif ($focus->bean_implements('ACL') && ACLController::requireSecurityGroup($focus->module_dir,
                'list')) {
            require_once 'modules/SecurityGroups/SecurityGroup.php';
            $owner_where = $focus->getOwnerWhere($current_user->id);
            $group_where = SecurityGroup::getGroupWhere($focus->table_name,
                    $focus->module_dir, $current_user->id);
...
function check_php_version($sys_php_version = '')
{
    ...
    if (version_compare($sys_php_version, constant('SUITECRM_PHP_MIN_VERSION'),
            '<') === true) {
        return - 1;
    }
    ...
    if (version_compare($sys_php_version, constant('SUITECRM_PHP_REC_VERSION'),
            '<') === true) {
        return 0;
    }
            ...
            if ($field_value != '') {
                foreach ($parms['db_field'] as $db_field) {
                    ...
                    if (DBManagerFactory::getInstance()->supports('case_sensitive')
                        && isset($parms['query_type']) && $parms['query_type'] == 'case_insensitive') {
    ...
    foreach ($seed->field_defs as $name => $field) {
        if ($field['type'] == 'relate' && isset($field['link']) && !empty($searchFields[$name]['value'])) {
            ...
            if (isset($field['db_concat_fields'])) {
                $db_field = db_concat($join_table_alias,
                    $field['db_concat_fields']);
...
function assignConcatenatedValue(SugarBean $bean, $fieldDef, $value)
{
    ...
    if (count($valueParts) == 1 && $fieldDef['db_concat_fields'] == array('first_name',
        'last_name')) {
...
function suite_substr($input, $start, $length = null,
                      $encoding = DEFAULT_UTIL_SUITE_ENCODING)
{
...
function suite_strpos($haystack, $needle, $offset = 0,
                      $encoding = DEFAULT_UTIL_SUITE_ENCODING)
{
...
function suite_strrpos($haystack, $needle, $offset = 0,
                       $encoding = DEFAULT_UTIL_SUITE_ENCODING)
{
...
function isValidEmailAddress($email, $message = 'Invalid email address given',
                             $orEmpty = true, $logInvalid = 'error')
{
    if ($orEmpty && !$email) {
        return true;
    }
    ...
    class ModuleRelatedField
    {
        ...
        protected function isModuleRelatedField($checked_field)
        {
            if ($checked_field['type'] === 'relate') {
                ...
                return false;
            } elseif ($checked_field['type'] === 'parent_type') {
                global $app_list_strings;
