...
class SearchForm {
   ...
   public function display($header = true) {
      ...
      $this->th->ss->assign(
              'searchTableColumnCount', ((isset($this->searchdefs['templateMeta']['maxColumns']) ? $this->searchdefs['templateMeta']['maxColumns'] : 2) * 2) - 1
      );
   ...
   public function _build_field_defs() {
      ...
      if ( $this->seed ) {
         ...
         foreach ( $this->seed->toArray() as $name => $value ) {
            ...
            if ( isset($this->fieldDefs[$fvName]['options']) && isset($GLOBALS['app_list_strings'][$this->fieldDefs[$fvName]['options']]) ) {
               ...
               if ( $this->fieldDefs[$fvName]['type'] == "parent_type" || $this->fieldDefs[$fvName]['type'] == "parent" || (isset($this->searchFields[$name]['options_add_blank']) && $this->searchFields[$name]['options_add_blank']) ) {
                  if ( !array_key_exists('', $this->fieldDefs[$fvName]['options']) ) {
                     $this->fieldDefs[$fvName]['options'] = array( '' => '' ) + $this->fieldDefs[$fvName]['options'];
                  }
               }
            }
            ...
            if ( isset($this->fieldDefs[$name]['type']) && $this->fieldDefs[$fvName]['type'] == 'function' && isset($this->fieldDefs[$fvName]['function_name'])
            ) {
            ...
            if ( (!empty($_REQUEST[$fvName]) || (isset($_REQUEST[$fvName]) && $_REQUEST[$fvName] == '0')) && empty($this->fieldDefs[$fvName]['function']['preserveFunctionValue'])
            ) {
   ...
   public function generateSearchWhere($add_custom_fields = false, $module = '') {
      ...
      foreach ( ( array ) $this->searchFields as $field => $parms ) {
         ...
         if ( !empty($this->seed->field_name_map[$field]['source']) && ($this->seed->field_name_map[$field]['source'] == 'custom_fields' ||
                 ...
                 ($this->seed->field_name_map[$field]['source'] == 'non-db' && (!empty($this->seed->field_name_map[$field]['custom_module']) ||
         ...
         if ( isset($parms['value']) && $parms['value'] != "" ) {
            ...
            if ( is_array($parms['value']) ) {
               ...
               if ( !empty($this->seed->field_name_map[$field]['isMultiSelect']) && $this->seed->field_name_map[$field]['isMultiSelect'] ) {
                ...
                $operator = !in_array($operator, ['subquery', 'subquery_with_in']) ? 'in' : $operator;
                  foreach ( $parms['value'] as $val ) {
                     if ( $val != ' ' && $val != '' ) {
                        if ( !empty($field_value) ) {
                           $field_value .= ',';
                    ...
                    case 'subquery_with_in':
                     ...
                     case 'isnull':
                        ...
                        case 'last_week':
                            ...
                            $formated_start_week = $start_week->format('Y-m-d H:i:s');
                            $end_week = $start_week->modify("next sunday");
                            $formated_end_week = $end_week->format('Y-m-d H:i:s');
                            $where .= "($db_field >= '$formated_start_week' AND $db_field <= '$formated_end_week')";
                            break;
                        case 'this_week':
                            global $timedate, $current_user;
                            $start = $timedate->now(true);
                        ...
                        case 'in_the_past':
                            ...
                            $where .= "$db_field < $formated_current_date";
                            break;
                        case 'in_the_future':
                            global $timedate, $current_user;
                            $start = $timedate->now(true);
                        ...
                        case 'last_n_days':
                            ...
                            $where .= "($db_field >= {$formated_date} AND $db_field <= {$formatted_current_date})";
                            break;
                        case 'next_n_days':
                            global $timedate, $current_user;
                            $start = $timedate->now(true);
                            ...
                            $formated_date = $modify_date->format('Y-m-d H:i:s');
                            $formatted_current_date = $current_date->format('Y-m-d H:i:s');
                            $where .= "($db_field >= {$formatted_current_date} AND $db_field <= {$formated_date})";
                            break;
                  }
               }
            }
