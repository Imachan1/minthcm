...
class CalendarUtils
{
    ...
    public static function get_time_data(SugarBean $bean, $start_field = "date_start", $end_field = "date_end")
    {
        ...
        $ccaDefs = CalendarActivities::getDefs($bean->module_name);
        if ($ccaDefs) {
            $start_field = $ccaDefs['start_time_field'];
            $end_field = $ccaDefs['end_time_field'];
            $arr['status'] = isset($bean->status) ? $bean->status : '';
        }
        // Mint - end
    ...
    public static function get_sendback_repeat_data(SugarBean $bean)
    {
        ...
        if (in_array($bean->module_dir, $cal_modules)) {
    ...
    public static function save_repeat_activities(SugarBean $bean, $time_arr)
    {
        ...
        // Mint - surrounded by if statement to avoid copying participants
        ...
        //Mint start
        if (count($meetings_to_sync) > 0) {
            $db = DBManagerFactory::getInstance("sync");
            foreach ($meetings_to_sync as $ids) {
                $m_id = $ids['meeting_id'];
                $u_id = $ids['user_id'];

                $user = BeanFactory::getBean('Users', $u_id);

                $sql = "Call sync_meetings_relationships_with_sales('{$m_id}');";

                $db->query($sql);

                $meeting = new Meeting();
                $meeting->retrieve($m_id);
                $meeting->update_vcal = true;
    ...
    public static function markRepeatDeleted(SugarBean $bean)
    {
        ...
        //Mint start
        $db_sync = DBManagerFactory::getInstance("sync");
        //Mint end
