...
class NewsController extends SugarController
{
    const NEWS_TPL = 'modules/Notes/tpls/News.tpl';

    public function action_getNewsTpl()
    {

        ob_clean();
        $filename = $this->getNewsTPLFilename();
        if(!empty($filename))
        {
...
tpl;
    ...
    protected function getNewsTPLFilename(){
        if(file_exists("custom/". NewsController::NEWS_TPL )){
            return "custom/". NewsController::NEWS_TPL;
        }
        if(file_exists(NewsController::NEWS_TPL )){
            return  NewsController::NEWS_TPL;
        }
        return '';
    }
