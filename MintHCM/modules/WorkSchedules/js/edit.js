        ...
        viewTools.api.callController({
            ...
            callback: function (call_constroller_data) {
                if ($.isEmptyObject(call_constroller_data) == false && call_constroller_data.result == false) {
                    result = false;
                    viewTools.GUI.fieldErrorMark($("#date_end"), SUGAR.language.get('WorkSchedules', 'LBL_WORKSCHEDULES_FOR_THIS_PERIOD_ALREADY_EXISTS'));
                }
            }
        });
    }
...
function validateWorkScheduleLength() {
    ...
    if (date_start != false && date_end != false) {
        ...
        if (diff_seconds > 24 * 60 * 60) {
            result = false;
            viewTools.GUI.fieldErrorMark($("#date_end"), SUGAR.language.get('WorkSchedules', 'LBL_WORKSCHEDULES_LENGHT_OVER_ONE_DAY'));
        } else if (diff_seconds <= 0) {
            result = false;
            viewTools.GUI.fieldErrorMark($("#duration_hours"), SUGAR.language.get('WorkSchedules', 'LBL_WORKSCHEDULES_DURATION_LESS_OR_EQUAL_ZERO'));
        }
        if (date_start.hours() == 0 && date_end.hours() == 0) {
            result = false;
            viewTools.GUI.fieldErrorMark($("#duration_hours"), SUGAR.language.get('WorkSchedules', 'LBL_WORKSCHEDULES_FORBIDDEN_HOURS'));
        }
    }
    return result;
}
...
function validateDatesEndAfterStart() {
    ...
    if (date_start != false && date_end != false) {
        ...
        if (diff_seconds < 0) {
            result = false;
            viewTools.GUI.fieldErrorMark($("#date_end"), SUGAR.language.get('WorkSchedules', 'LBL_WORKSCHEDULES_DATE_END_BEFORE_START'));
        }
    }
    return result;
}
...
function createActivitySelectDialog() {
    $("body").prepend('<div id="activity-select-dialog"><p><b>' + SUGAR.language.get('WorkSchedules', 'LBL_ACTIVITY_SELECT_DIALOG') + '</b></p></div>');
    ...
    $("#activity-select-dialog").dialog({
        ...
        buttons: [
            ...
            },
            {
                ...
                click: function () {
                    var url = "index.php?module=Calls&action=EditView&return_module=" + return_module + "&date_start=" + $("#date_start").val() + "&assigned_user_id=" + $("#assigned_user_id").val();
                    window.location.assign(url);
                }
            }
        ],
    ...
    if (type !== "") {
        ...
        viewTools.api.callCustomApi({
            ...
            callback: function (data) {
                if (data) {
                    var error_label = SUGAR.language.get('WorkSchedules', 'LBL_WORKSCHEDULES_PERIODICITY_CREATE_ERROR') + " " + data;
